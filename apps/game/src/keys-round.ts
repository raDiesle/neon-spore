 ...
 * (`docs/spec/interludes.md`), so THE GAUGE and THE FLEET each have their own
 * commands — and at a desk that means their own keys. Split out of `keys.ts`
 ...
 * R fires the pilot's salvo into whichever square they are standing on.
 */
export function roundKeyDown(code: string): Press | null {
  switch (code) {
    case "KeyZ":
