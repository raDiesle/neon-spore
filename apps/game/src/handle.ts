...
export interface HandleParts {
   ...
   * whole field, which a caller stepping ticks cannot see or wait out. */
  launching: () => boolean;
}
...
export function installTestingHandle(parts: HandleParts): void {
  ...
  (window as unknown as { neonSpore: unknown }).neonSpore = {
     ...
     * `bun run frames` took every capture it has ever taken through them,
     * still there 2500 ticks into a wave. Painting is what moves them, and
     * this is how a caller knows when to stop (`tools/frames/opening.ts`).
     */
