...
import { drawTrivetFace, drawTrivetSockets } from "./trivet-marks.js";
import {
  trivetArrived,
  trivetAsksFoot,
  trivetBuckle,
 ...
 * the hub's face smaller and brighter for every hit. Nothing here outlives a
 * frame.
 */
export function drawTrivet(
  ...
  ctx.translate(home.x, home.y - trivetDrop(l, arrived));
  ...
  for (const side of [0, 1] as const) {
    ...
    drawPlate(ctx, l, s, step, side, foot, beatPhase);
    ctx.globalAlpha = alpha;
  }
...
/** An outer foot's plate, turned with its leg: its sockets, and the clamp across its ankle as it locks. */
function drawPlate(
  ctx: CanvasRenderingContext2D,
  l: Layout,
  s: TrivetState,
...
/** SINKER's hub in gunmetal, the key light on it and its pale rim. */
function drawHub(ctx: CanvasRenderingContext2D, l: Layout, time: number): void {
