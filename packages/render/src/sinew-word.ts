...
/**
 * **What THE SINEW is asking of one hand**, and the silences that are the
 * fight.
 *
 ...
 *
 * **Three moments it may speak in, because none of them leaks a number.**
 *
 ...
 *   is already out has nothing to read otherwise.
 *
 ...
 * gone and the sum is not read at all (`sinew-step.ts`) — then the swing, whose
 * word is the same for both seats, then the slack, which speaks to a hand on
 * and silences a hand off, and last the hand itself: off the ring there is only
 * the pull to offer, and on it the hold and otherwise the pair's own
 * conversation.
 */
