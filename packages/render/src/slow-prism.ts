...
import { beatFrac, drawAbout, inField, snapshot } from "./slow-lens.js";
import type { SlowWindow } from "./slow-look.js";

/**
 * **PRISM — the room splits into its colours, and the boss does not.** The
 * owner took it from the `slow:pull` slot on 26 September 2026, *on top of*
 * the streams rather than instead of them (`tools/versus/DECIDED.md`).
 *
 * The field is pulled apart channel by channel about the boss's head: red
 * swells outward, blue falls inward, green stays. A point's fringe is as wide
 * as it is far from the boss, so the boss itself stays sharp and the edges of
 * the room tear into red and blue — a lens bent round the thing the window is
...
/** Splits the field about `at`, as far as `up` has brought the look in. */
