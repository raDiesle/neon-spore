 ...
 * three chevrons for the swipe, a hooked arc for the turn, two thumbs for
 * the hold. Each is drawn in the caller's stroke colour at the ring's
...
export function drawInstarGlyph(
  ...
  else if (gesture === "turn") turn(ctx, x, y, r, time);
  else hold(ctx, x, y, r);
  ctx.restore();
}
...
function turn(ctx: CanvasRenderingContext2D, x: number, y: number, r: number, time: number): void {
  const spin = (time * 2) % (Math.PI * 2);
  const a0 = spin;
  const a1 = spin + Math.PI * 1.5;
  ctx.beginPath();
  ctx.arc(x, y, r * 0.45, a0, a1);
  ctx.stroke();
