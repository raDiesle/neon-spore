 ...
 * since the first is a new kind of thing a film can point at — while the box
 ...
 * the ghost hand plays by (`guide-thumb.ts`).
 */
...
export function anchorPoint(
  ...
  if (anchor.at === "ship") {
    // The swelling on the hull this control is reached through. `shipCircle`
    // is the one place that pairing is decided, so the ring cannot land on one
    // swelling while the hand presses the other.
  ...
  // A place and a cost, drawn in one spot: the hull is where a hit lands
  // (`scene-step-types.ts` says why the two are kept apart).
  return { x: l.width / 2, y: l.hullY, r: l.tile, clear: CLEAR };
}
