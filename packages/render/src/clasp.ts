import { crystalUnder, type SimConfig, type World } from "@neon-spore/sim";
...
export function claspResonanceIn(world: World): number {
  for (const c of world.creatures) {
    ...
    // And THE CRYSTAL, the same link one creature on: the shield standing
    // under any lane of it is the half of its answer player 2 holds.
    if (c.kind === "crystal" && crystalUnder(world, c)) return 1;
  }
  return 0;
}
