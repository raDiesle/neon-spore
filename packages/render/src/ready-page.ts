...
export function drawReadyPage(
  ...
  drawIntroduction(ctx, l, world, fx?.age ?? Number.POSITIVE_INFINITY, false);
  ...
  // Three rows above the button, and the room between them is the whole of what
  // keeps a circle's name off the line under the one that names who is waiting.
  waitingLine(ctx, l, box.y - 100, iAmReady, theirs, both, other, fx?.age ?? 0);
  ...
  button(ctx, box, iAmReady, fx?.age ?? 0);
...
/** The button, held rather than tapped. Lit while it is being held down. */
function button(ctx: CanvasRenderingContext2D, box: NavBox, done: boolean, age: number): void {
