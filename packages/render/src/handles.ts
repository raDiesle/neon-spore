 ...
 *
 * There are two of them now — THE MAZE's string and THE WARDEN's rope — and
 * that is why they are here rather than in `touch.ts` next door. Both answer
...
export function handleUnder(l: Layout, x: number, y: number, field: Field): Touch | null {
  return mazeStringUnder(l, x, y, field) ?? wardenRopeUnder(l, x, y, field);
}
