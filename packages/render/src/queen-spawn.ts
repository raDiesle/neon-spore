 ...
 * moves on with the rock from then on. Only `spit()` ever makes a meteor, so
 * this heuristic never needs to tell her rocks apart from another source.
 */
export function isFreshQueenSpawn(c: Creature, origin: QueenOrigin | null): boolean {
  if (origin === null || c.kind !== "meteor") return false;
...
export function queenSpitSide(creatures: readonly Creature[], origin: QueenOrigin): -1 | 0 | 1 {
  for (const c of creatures) {
    if (c.kind !== "meteor" || c.fromRow !== origin.row) continue;
