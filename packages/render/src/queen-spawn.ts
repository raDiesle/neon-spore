 ...
 * `meteorFast`, so this heuristic never needs to tell her rocks apart from a
 * wave-authored one of a different kind.
 */
export function isFreshQueenSpawn(c: Creature, origin: QueenOrigin | null): boolean {
  if (origin === null || c.kind !== "meteorFast") return false;
...
export function queenSpitSide(creatures: readonly Creature[], origin: QueenOrigin): -1 | 0 | 1 {
  for (const c of creatures) {
    if (c.kind !== "meteorFast" || c.fromRow !== origin.row) continue;
