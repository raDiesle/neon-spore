...
/**
 * SCATTER — THE WISP's strike: it goes out like a light, not a body.
 *
 * The wisp keeps no spores of its own — alive it draws as two dots, the same
 * as everything without an answer of its own (`body-interior.ts`'s
 * `interiorFor`) — but this strike needed eleven points that would scatter
 * without reading as a smudge at twenty-six pixels, and the bulb's own
 * packing (`body-spores.ts`) had already solved exactly that: eleven spheres
 * pinned three deep through a body's volume. Borrowed for the positions
 * alone, not for what they mean — nothing sat at them a beat ago. On the
 * beat the contour flashes and is gone, and from each of those eleven
 * borrowed points a lit ball of the shot's own colour flies outward, the
 * deep ones slower and dimmer than the near ones, every one drifting **up**
 * as it slows, because a mote of light is lighter than the field. Each
 * trails a short thread of its own light, and they blink out one by one over
 * the strike, the last of them well above where the wisp stood.
 *
 * It leaves nothing on the ship: light rises. What it leaves instead is the
 ...
 *
 * The flash is the rim colour, the motes are the shot's own colour pushed
 * toward its rim by how near they were, and the threads are the same at a
 * fraction. A wisp has no colour of its own (`creatures-split.ts`: either
 * kills it), so the shot's colour and the strike's are the only colour here.
 *
 * **How it can lose.** *Eleven lit balls in a lane are eleven bodies.* For
 * the first tenth of a second the motes are still inside the contour and
 * read as the wisp; a moment later they are a cloud, and a cloud in a lane
