 ...
 * (`frame.test.ts`). Whether the shell reads as an hourglass or the light under
 * the join says "the shield is here" needs a phone; what this holds is that a
 * whole one paints on both screens, that the beat it is opened on and the two
 * bodies after it paint too, and that the link the ship's arcs read is on
 * exactly when the shield stands under the *middle* tile.
 */
...
describe("a crystal on the field", () => {
  ...
  it("paints the beat it is opened on and the two bodies after it", () => {
    // Entered at column 0, it stands on columns 4..6 through the fourth beat
    // with its middle on 5; the shield and the cannon go there, then the guard
    // and the shot land on the first tick of that beat, and the bolt meets the
    // join before the body moves on (`sim/test/crystal.test.ts` has the rule;
    // this only has to paint the split and the two bodies after it).
    const at = TPB * 3 + 1;
    const inputs: TimedCommand[] = [
      { tick: at - 10, player: 2, command: { kind: "shieldCol", col: 5 } },
      { tick: at - 10, player: 1, command: { kind: "cannonCol", col: 5 } },
...
describe("the link the ship's arcs read", () => {
  it("is on under the middle tile and off under either end", () => {
