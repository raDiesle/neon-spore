...
import { showsGrateGaps } from "../src/grate.js";
import { computeLayout, type ViewRole } from "../src/layout.js";
import { CFG, installCanvasGlobals, ROLES, runFrames, VIEWPORT } from "./frame-harness.js";

/**
 * THE GRATE, drawn: the wall both seats see, and the breaks only the pilot has.
 *
...
function grateFrames(role: ViewRole, gaps: number[], ticks: number) {
  const queue: SpawnEntry[] = [{ beat: 0, col: 0, kind: "grate", color: null, gaps }];
...
describe("the grate", () => {
  ...
  for (const role of ROLES) {
    it(`draws the wall and its breaks for ${role}`, () => {
      const { ctx } = grateFrames(role, [4], TICKS);
  ...
  it("keeps the canvas happy with the way through against either wall", () => {
    for (const gaps of [[0], [CFG.cols - 1], [0, CFG.cols - 1]]) {
      const { ctx } = grateFrames("p1", gaps, TICKS);
      expect(ctx.calls).toBeGreaterThan(1000);
    }
  });
  ...
  it("gives the two seats two different pictures of the same wall", () => {
    ...
    const p1 = grateFrames("p1", [4], TICKS);
    const p2 = grateFrames("p2", [4], TICKS);
    expect(p1.ctx.calls).not.toBe(p2.ctx.calls);
  });

  it("shows the gaps to the pilot and the rig, and never to the navigator", () => {
    for (const role of ROLES) {
      const l = computeLayout(VIEWPORT, CFG, role);
      expect(showsGrateGaps(l), role).toBe(role !== "p2");
    }
  });
});
