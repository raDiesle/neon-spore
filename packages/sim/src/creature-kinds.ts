...
export type CreatureKind =
   ...
   * the beat, and commits the run by *stopping* rather than finishing. The
   * number is drawn on player 1's screen alone, so the seat that can read it
   * is the seat with no thumb on the body. Right and it is silenced; wrong
   * and it fires a wave of sound at the hull and keeps falling.
   * `Creature.beatboxWant`, `beatboxHits` and `beatboxBeat` are its state;
  ...
  // domes. `creature-kinds-many.ts` next door, cut out on purpose so this file
  // has room for the next creature (`docs/queue.md`, 6 September 2026).
  | ManyKind;
