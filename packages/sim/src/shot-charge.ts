 ...
 *  - Each of a Throb's two halves is square to the cannon for half of every
 *    turn, a beat and a half at `throbSpinBeats`. A half-beat grid puts three
 *    departures inside that window, so a colour called out loud can still be
 *    fired before the body has turned it away; a whole-beat grid leaves one,
 *    and an answer that arrives after the turn is not an answer.
