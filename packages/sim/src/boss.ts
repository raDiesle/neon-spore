...
export function stepBoss(world: World): void {
  ...
  // THE GAUGE, SNAKE, PINBALL and THE CLAW never reach this. All four are
  // stepped on the tick from `step`'s own early return, and the field's beat
  // does not run while any of them stands — so the branch is here to say that
  // out loud rather than to do anything.
  if (
    boss.kind === "gauge" ||
    boss.kind === "snake" ||
    boss.kind === "pinball" ||
