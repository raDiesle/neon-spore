...
export function armShield(world: World): void {
  ...
  // THE COIL is deliberately **not** here, and it is the one thing the shield
  // does that is not an edge. A clasp is opened by the moment the trigger
  // arrives; a dome is opened by the plate *standing under it*, whichever of
  // the two got there second, so it is asked of the open window every tick in
  // `step` instead (`wardCoils` in `coil.ts`). Arming it from here as well
  // would be a second copy of that rule, and the copy is the one that would go
  // on answering a press the window had already closed on.
}
