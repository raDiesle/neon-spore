import type { InstarPart, InstarPose } from "./instar.js";
...
export type InstarEvent =
  ...
  | ({ type: "instarMorph"; step: number; pose: InstarPose } & InstarColEvent)
  ...
  | ({ type: "instarAnswer"; mark: number; part: InstarPart } & InstarColEvent)
  /** Mark `mark` reached its need: the part gives — the hand opens, the tail lifts. */
  | ({ type: "instarDone"; mark: number; part: InstarPart } & InstarColEvent)
  /** Mark `mark` was done and its partner was not in time: back to nought. */
  | ({ type: "instarSlip"; mark: number; part: InstarPart } & InstarColEvent)
  ...
  | ({ type: "instarStrike"; part: InstarPart } & InstarColEvent)
