 ...
 * (§20, row 7).
 *
 * Its own file beside `hasp-step.ts` for `bellows-shot.ts`' reason — next
 * door is the fight's clock, and this happens where a bolt leaves the top of
 * the field. The hasps themselves are rock grey and none of them is ever
 * shot: the cannon has one thing to do in this whole wave and the shield has
 * none at all, which is what makes the one thing worth keeping on the band.
 *
