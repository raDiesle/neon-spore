...
/**
 * **The outline of a metaball field, as however many closed loops it has** —
 * the machinery SYMBIOSIS, the Colony and THE CHOIR are all drawn with.
 *
 * Moved here from `tools/shape-sheet/src/iso.ts` when the game itself needed
 * it: THE CHOIR is two bodies in one membrane on the field, and the owner asked
 * for the *Symbiosis graphic* rather than an approximation of it. `content` is
 * where a shape the game draws and the sheet draws belongs — the argument
 * `body-path.ts` next door makes about `livingPoints`, one description and two
 * drawings. The sheet's `iso.ts` re-exports every name here.
 *
 * A radial march — at each angle, the distance where the field crosses the
 * threshold — is cheaper and can only ever return *one* outline, because one
 * angle has one answer. Marching squares has no such assumption: it walks a
 * grid and joins the crossings into loops, one while the bodies are merged and
 * several the instant they are not, with the parting happening on its own.
 *
 * **`Math.hypot` and friends are legal here and not in `sim`.** That ban is
 * scoped to `packages/sim/src`, because a transcendental that is not correctly
 * rounded can desync two phones through a stored integer. Nothing here is
 * stored: it is a contour, thrown away every frame.
 */

/** A box in the field's own units. */
...
export function perimeter(loop: Point[]): number {
  let l = 0;
