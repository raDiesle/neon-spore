 ...
 * the beam standing in its way, twice.
 *
 ...
 * dead, then makes a pass that only the beam standing in its column stops —
 * the first beam dead where it met it, the second for good.
 ...
 * is down, is warded by the same shield with a second guard. The third, at
 ...
 *
 * **The pass and the two beams** (`leadStillFills`, doubled on the owner's
 * rule, 24 September 2026). With one segment the body stops dead at the
 ...
 * columns a beat: 10 to 7, across 8, and the body stops dead in 8 where the
 * beam met it — a whole new still. The second beam is in the same column,
 * held three beats before that still runs out, and the pass leaving 8 is down
 * the beat it starts. Every page is on a control or on the hull: the body is
 * a fixture and no anchor names one (`docs/queue.md`, the gauge item).
 */
export const THE_LEAD: GuideScene = {
  ...
  acts: [
    ...
    { tick: 1220, control: "fireRed", until: 1490 },
    // The rock dropped in column 2 on beat 8, landing on beat 21.
    { tick: 1230, control: "guard" },
    // The rock dropped in column 6 on beat 12, landing on beat 27.
    { tick: 1540, control: "shield", col: 0, atBody: true },
    { tick: 1590, control: "guard" },
    // Stopped dead in 8, where the beam met it: the second, in the same column.
    { tick: 1760, control: "fireRed", until: 2030 },
  ],
  steps: [
    ...
    },
    {
      tick: 1260,
      seat: 1,
      text: "ITS ROCKS STILL FALL",
      anchor: { at: "control", control: "guard" },
    },
    {
      tick: 1680,
      seat: 2,
      text: "STOPPED · ONE BEAM MORE",
      anchor: { at: "control", control: "fireRed" },
    },
  ],
};
