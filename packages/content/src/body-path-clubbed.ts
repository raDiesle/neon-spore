...
/**
 * The one walked contour: a rim of balls on stalks. `body-path.ts` routes a
 * silhouette here when it wears clubs and samples a blob otherwise, and this
 * file is the walk and the four numbers it is sampled at — moved out whole
 * when the router reached its limit, because a walk grows on its own reasons
 * (THE BEATBOX's growing club, the floor under a gap) and the route does not.
 */
