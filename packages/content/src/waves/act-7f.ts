...
/**
 * The sixth page of act seven, cut off `act-7d.ts` when THE LEAD would have
 * taken it over the 250-line ceiling: THE SURGE had left it three under.
 *
 * **`7f` and not `9`, for the reason `act-7c.ts` gives about `7c`**: an act
 ...
 *
 * **It was `act-7e.ts` until 19 September 2026**, when THE TASTER and THE
 * SINEW — `act-7d.ts`'s own last two waves, and earlier in the order of the
 * game than everything on this page — needed a page of their own between
 * `act-7d.ts` and this one. Taking this page's own letter would have played
 * them after THE INSTAR, seven waves later than the game has always played
 * them, so this page's letter moved instead and its content did not.
 *
