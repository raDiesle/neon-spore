...
const STILL_PROSE = [
  ...
  "THE VALVE",
  // And THE SEAM (§26), a tenth time: the ridge is undrawn, and the guide
  // says which control answers which step.
  "THE SEAM",
  // And THE OCULUS (§27), an eleventh time: the eye is undrawn, and the guide
  // says which thumb holds which leaf.
  "THE OCULUS",
...
describe("what `docs/spec/briefings.md` §3.2 says about the rehearsals", () => {
  ...
  it("counts the guided waves the opening section names", () => {
    ...
    const fix =
      "update §1 of docs/spec/briefings.md, which says one hundred and three of the hundred and fourteen";
    expect(guided.length, fix).toBe(103);
    expect(WAVES.length, fix).toBe(114);
  });
