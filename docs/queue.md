...
## THE LEDGER IS DIRTY IN EVERY FRESH CHECKOUT, AND THE GUIDANCE ABOUT IT IS BACKWARDS
_claude/burn-ledger-line-endings · docs/verified.md .gitattributes .claude/skills/autonomous/SKILL.md docs/verification.md tools/checks/ledger.ts tools/checks/test_
**Proposed by the run.**

**Found while landing, not theorised.** Marking the outstanding checks tested
on 28 August 2026 ran into all of this at once, and the evidence is three
commands rather than an argument:

- `.gitattributes` says `* text=auto eol=lf`.
- `git cat-file -p HEAD:docs/verified.md | grep -c $'\r'` returns the file's
  whole line count. The **blob carries CR bytes**.
- `git checkout -- docs/verified.md` therefore writes a file with none, which
  no longer matches the blob — so the file comes back **modified in a tree that
  has just been checked out clean**, and stays that way.

That last line is the actual defect and it is not cosmetic. A permanently dirty
ledger is why a *truncated* copy of it sat in this working tree unnoticed: 59
lines against the committed 200, missing the owner's own note about the rock
going into the ship on the cannon's column. Recording verdicts against that
copy reported 106 outstanding checks instead of 25, because eighty-one already
decided ones had lost their rows. A file that is always modified is a file
whose modification nobody reads.

**And the written guidance now disagrees with the file twice.** The "what this
has already got wrong" section of `.claude/skills/autonomous/SKILL.md` says
`docs/verified.md` is LF, that `a9f4755` settled it, and that writing a CRLF
row breaks `ledger.ts` because `(.*)$` cannot reach past a carriage return.
The blob is CRLF and `bun run checks` parses it — twenty-five rows were
appended that way and the list closed correctly. So a lane that trusts the
skill file will make exactly the wrong repair. Whichever way this is settled,
that paragraph is rewritten against what the file actually is.

**Decide the endings once, and the decision is the lane's real work.** Two
honest answers and they are not equally cheap.

*Normalise to LF.* Consistent with `.gitattributes` and with every other file
in the repository. Costs one commit that rewrites all 238 rows — a diff nobody
can read, in the one file whose whole value is being an append-only log, and
`ledger.ts`'s header says so in as many words. Do it and the log's history has
a scar across it forever.

*Keep CRLF and say so.* One line in `.gitattributes` exempting the path, and
the file stops being dirty on checkout with no rewrite at all. It makes the
ledger the one file that is not LF, which wants a sentence in `ledger.ts`'s
header explaining why rather than a future contributor "fixing" it back.

**Weigh them by what goes wrong, not by tidiness.** Also settle what
`ledger.ts` actually tolerates rather than repeating either claim: a test that
parses a CRLF row and an LF row and asserts both round-trip is the thing that
stops this being re-derived wrongly a third time. That test is the deliverable
even if the endings decision goes the other way.

**Do not tidy the file while you are in it.** Duplicate rows are a defect in
whatever wrote them, and the fix for a duplicate is in the writer.

This is a tool fix, not a look: nothing the game draws changes.

Finished when `bun run check` is green, a fresh `git checkout` of
`docs/verified.md` leaves the tree clean, `ledger.ts` has a test pinning what
it parses, and the skill file and `docs/verification.md` say what is true. The
commit needs no `Check:` trailer — there is nothing here a player could look
at, and the right number of trailers for that is zero.

Model `sonnet`, effort `think hard`. Read `.gitattributes`, `tools/checks/ledger.ts`
in full, and the "what this arrangement has already got wrong" section of
`.claude/skills/autonomous/SKILL.md`. The thinking goes on the endings
decision and on writing the paragraph that replaces the wrong one — a note
that says what was checked and how is what stops a fourth session guessing.
