...
## THE TRANSPORT ROW SHOULD BE ONE ROW, AND THE BALANCE SHEET SHOULD FOLD AWAY
_claude/burn-transport-row · tools/director/index.html tools/director/src/pair-panel.ts tools/director/src/stage-transport.ts tools/director/src/balance.ts tools/director/test/stage.test.ts_
**Asked for by the owner.** **This is the next thing done.**

> the "briefings" checkbox should be same visual and same line then other
> buttons below gamescreen like "wave" "sheet" "card" ( no checkbox visual, but
> works like checkbox, so it stays active until you click again).
>
> the balance sheet content should be hidden behind a button like "sheet".

**This is a correction to what landed an hour ago**, not a new idea. The layout
lane moved the Briefings control out of TUNING → PAIR and put it under the
field, which was right, and left it as a checkbox in its own `<div>` on its own
line, which was not. The same lane made the balance sheet a live panel under
the stage, always open. Both are one step from what the owner wants.

### One row, one kind of button

`.transport` (`index.html`) already carries `⏸` (`#play`), `↺ WAVE`
(`#restart`), `▣ SHEET` (`#endRun`), `✓ CARD` (`#ackBrief`) and the three role
buttons. `#briefToggle` sits *below* it as a full-width mount that
`pair-panel.ts` fills with a checkbox row.

**Briefings joins that row and looks like the rest of it.** No checkbox, no
label beside a box — a button, in the same line, in the same style. It **works**
like a checkbox: pressed once it is on and stays on, pressed again it is off.
So it needs an on-state the eye can read at a glance, and the row already has
the convention for that — the role buttons carry `class="role on"` and the play
button already swaps `▶`/`⏸` to say what the next press does. Use whichever of
those two the row makes look deliberate, and say which in the commit.

**`✓ CARD` stays exactly as it is and is not merged into this.** It is a
*dismiss* — it pushes `{kind: "brief"}` for both seats — and without it, turning
briefings on freezes the stage on the first wave forever, because `startWave`
opens the card and nothing else in the director can put it away. Two controls
that sound alike and are not: one says whether cards appear, the other puts the
current one away. `stage-transport.ts`'s own comment says so; do not tidy it.

**`pair-panel.ts` still owns the other two PAIR rows** and mounts this one. Keep
that arrangement — the lane changes what the Briefings row *is*, not who builds
it.

### The balance sheet folds away

`#balanceSheet` and its heading and note sit in `#stageBelow`, always drawn.
Put them behind a button in the same row, working the same way as the Briefings
one — press to show, press again to hide. The owner names `▣ SHEET` as the
model, and that button is right there in the row, which is the point: the two
should look like siblings.

**Mind the name collision.** `▣ SHEET` already means *the after-run screen* — it
ends the run and shows what the pair scored, and a second press brings the field
back. A button beside it also called "sheet", meaning the live balance panel,
is two different sheets in one row. Name the new one so a person can tell them
apart at a glance, and say in the commit why you chose that word.

**Remember what was fixed on the way in.** The stage column's `overflow:
hidden` was clipping the field's own note text, and the layout lane repaired it.
Do not reintroduce it while making a panel collapsible.

### Not a look being replaced

This is the director, and the owner asked for both changes by name. Nothing the
game draws changes.

Finished when `bun run check` is green, both toggles keep their state until
pressed again, the row reads as one row, and the commit carries

`Check: below the game screen, do the buttons now read as one row of the same kind of control, and do briefings and the balance sheet each stay put until you press them again`

Model `sonnet`, effort `think`. Read `tools/director/index.html`'s `.transport`
block, `tools/director/src/pair-panel.ts` and
`tools/director/src/stage-transport.ts`. The layout is the easy half; the
thinking goes on the two naming collisions — *sheet* against *sheet*, and
*card* against *briefings*.
