...
The owner asked for this by name on 17 September 2026, which is the first of
the three look exemptions — but he asked for **candidates**, so every picture
below goes to VERSUS and none of it goes straight onto the field. His words:
*create me more "versus" alternative animation graphics, which make the "game
over" screen more attractive. the current structure is ok, but needs visual
pimping.* Start from the current trunk; he said that too.

What he asked for, item by item:

- **WAVE LOST sits too high.** It is stamped at `playHeight * 0.16`
  (`lost-shutters.ts`, `words`), and he wants it further down, nearer the held
  field. The buttons are at `0.52` and `lost-look.ts` forbids moving them, so
  the room is between the two.
- **The violet rivulets are the thing to argue with.** *right now we have
  those dots with tail in purple falling down. make me some alternatives of
  that in particular* — that is `lost-blood.ts`, thirteen runs of the hull's
  own violet crossing the screen top to foot. It is the newest thing on this
  screen and the one he is least settled on.
- **The damage has to stay visible where it happened.** *i guess the damaging
  animation of where it hit should stay during the "wave lost" screen* — so a
  candidate replays the breach at `breachX`: the crest of `breach-hammer.ts`
  **once**, not looping, and the char of `breach-rend.ts` over it, and the
  char **holds for the whole of the lost screen** rather than fading with the
  animation that put it there. Both were adopted on 16 September 2026 and are
  what he means by *the wave animation* and *the dark glow*.
- **Heavy damage, and red.** *maybe colour also in red or let it see some
  heavy damage across the full area, maybe blooding out in red in all
  directions.* `lost-blood.ts`'s own header argues at length for violet over
  red and names `HUE` as the single knob — that argument is answered by the
  owner asking, so a red answer is legitimate and the header is what it should
  quote rather than contradict silently.

**Split before it is started, and the halves land separately.**

*Half one, and not one pixel moves:* `LostPaint` can say where the ship was
hit and nothing else about it. Replaying either breach needs what
`StrikePaint` takes — `span`, `hex`, `seed` and `surfaceY` — so the record
gains them (or a `breach: StrikePaint | null` whole, which is likelier to read
well), `lost-screen.ts` fills them from the world, and a helper runs a strike
off the lost screen's own `age` with the char held past the end. A wave with
no scar still hands `null`, which is `breachUnscarred` and already a case
every answer must have.

*Half two:* the `lost:screen` slot opened again with three or four answers,
each a whole `LostLook` (`veil` and `words`), registered with
`bun run versus index` and given its pose row back in
`tools/director/src/versus-pose.ts` — the row went with the slot when
`shutters` was taken (`tools/versus/DECIDED.md`). One answer per argument, not
four tunings of one: the held breach; the full-field red bleed-out; something
that is not liquid at all; and the shipped violet left standing as the
left-hand side, which is read off the live record and never given an entry.

Read `tools/versus/README.md` and `DECIDED.md` first — `lost:screen` has been
opened and closed once already, and `adopt` cannot write a `paint` back into a
record, which is why `shutters` was taken by hand. `docs/looks.md` is the rule
