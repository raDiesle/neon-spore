...
## LEDGER SAYS WHAT SHEET ALREADY SAYS, AND THE FIELD HAS BEEN SQUEEZED
_claude/burn-stage-room · tools/director/index.html tools/director/src/balance.ts tools/director/src/stage-transport.ts tools/director/test/transport.test.ts_
**Asked for by the owner.** **This is the next thing done.** Both halves are
corrections to landings from the last two hours, made while looking at the
result.

> seems below the game screen: "sheet" and "ledger" is the same info. remove
> "ledger" the "sheet" is good enough.
>
> the game screen width and height is now very small, make it bigger, so ideally
> buttons are shown on bottom of my pc screen, to get most reasonable space of
> game screen available.

### LEDGER goes, and the entry that added it was warned about this

`▤ LEDGER` was added an hour ago to fold the live balance panel away, and the
owner is right that it duplicates `▣ SHEET`. The panel's own note in
`index.html` says so in as many words: *"The same numbers the screen after the
run draws, moving as the wave is played."* Same numbers, one live and one after
the run — and a second button in the row to hide a thing that says what the
button beside it already says is one control too many.

**Remove the button and the live panel, not just the button.** A hidden panel
that nothing can open is dead markup, and the `on`-state wiring in
`balance.ts` goes with it.

**But check what else uses `balance.ts` before deleting anything.**
`sheetLines` and `sheetMemories` are exported and the after-run screen is drawn
by `packages/render/src/balance.ts`, which is a different file with a similar
name — do not confuse them and do not touch the render one. If the director's
`bindBalance` turns out to have another caller, keep what is needed and say so.
`bun run check` and `tools/orphans` are the guards against leaving a corpse.

**The test that just landed covers LEDGER's toggle.** `transport.test.ts` pins
that LEDGER stays on until pressed again. That case goes with the button; the
BRIEFINGS half of the same file stays and must keep passing.

### The field needs the room back

The stage is *the reason the tool exists* — it is the shipping renderer against
a real world, and the one question the data cannot answer is whether the cannon
has time to get there. A field too small to judge that is a director that has
stopped doing its job.

**Where the space went.** `#stageWrap` is `flex: 3 1 0` and `#stageBelow` is
`flex: 2 1 0`, so the column below the field takes two fifths of the height no
matter how little is in it. With the balance panel folded away — and now
removed — most of that two fifths is a paragraph of note text.

**What the owner asked for is a specific arrangement**, and it is the one to
build: the transport row sits at the **bottom of the screen**, and the field
takes everything above it. So the buttons are pinned rather than floating in
the middle of a column, and the space they used to share goes to the stage.

**The field keeps its shape.** It is a phone-shaped rectangle — `computeStage`,
the same call the device makes — so growing it means growing it *within that
aspect*, and on a wide PC screen the limit will be height rather than width.
Say the measured before-and-after in the commit: the field's drawn size in
pixels at a typical desktop window, before and after.

**Do not reintroduce `overflow: hidden` on the stage column.** It was clipping
the field's own note text and was repaired this morning; re-flowing this column
is exactly where it comes back.

**The note text is not sacred.** If the paragraphs under the field are what is
eating the room, they can move, collapse or shorten — they explain the tool to
somebody reading it once, and the field is used every day.

This is the director, and both halves were asked for by name. Nothing the game
draws changes.

Finished when `bun run check` is green, `▤ LEDGER` is gone with its panel and
its test case, `transport.test.ts` still passes for BRIEFINGS, and the commit
carries

`Check: on your own screen, is the field now big enough to judge whether the cannon reaches a column in time, with the buttons sitting along the bottom`

Model `sonnet`, effort `think`. Read `tools/director/index.html`'s `#stageWrap`
and `#stageBelow` rules and `tools/director/src/balance.ts` in full. The layout
is the work; the deletion is bookkeeping. Measure the field before you start so
the commit can say what it grew from.
