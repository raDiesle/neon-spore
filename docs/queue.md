...
The owner reports CILIA as very slow, and the cause is in one loop:
...
`RIM_COUNT` is 100, that runs every `onFrame`, and the SHAPES tab draws sixty
cards — so a frame makes **about six thousand `getPointAtLength` calls**.
That is one of the most expensive calls in the SVG DOM: each one can force the
browser to re-parse the path and rebuild its arc-length parameterisation, and
the path's `d` has just been rewritten that same frame, so nothing is cached
between them.

**This was an honest choice and it is worth saying so**, because the fix is not
"the lane was careless". A fringe has to stay welded to a contour that reshapes
every frame, `SkinContext` gave a skin no way to know its own body, and the
`<path>`'s own geometry methods were the one thing that could not go stale.
The header says exactly that. The mistake is not the ruler, it is asking the
DOM for it.

**The contour is arithmetic and the browser is not needed.** `contourAt` builds
that `d` from points computed in JavaScript, and `pointsAt` in
`tools/shape-sheet` already hands them over — `transformedBounds` walks them.
A skin that has the points interpolates between two of them with two
multiplies, instead of asking a layout engine to solve arc length. That is a
change of kind, not a percentage.

**It composes with `claude/burn-body-context-s14`**, which is closing the very
gap that produced this: a skin is told its reach and never its shape. If that
lane has landed, take the points from whatever it added and delete the ruler
path entirely. If it has not, take them the way `WIND` took its extent, and say
in the commit which caller `s14` will need to clean up.

**If the points genuinely cannot be reached**, the fallback is still large:
sample the ruler **once** per frame into a small table — sixty-four points is
finer than a hundred strands can show — and interpolate the hundred out of it.
That is 64 DOM calls instead of 100 and, more to the point, it is a table you
can reuse rather than a hundred independent solves.

**Measure it, in the shape the fit-memoisation lane measured its own win**:
time a frame's worth of `onFrame` before and after, on the full sixty-card tab,
and put both numbers in the commit. A claim about speed is a number or it is
nothing. Watch the whole-page figure too — a lane already recorded that PORE
draws about 38,200 elements over the sixty cards, and if CILIA stops being the
bottleneck the next one should be named rather than left to be rediscovered.

**Do not change how it looks.** The strand count, the seeded scatter, the
travelling phase offset and the lean all stay exactly as they are; this is the
same fringe drawn by cheaper means. Prove it the way three lanes have now
proved it: build every catalogue entry at several poses and hash the DOM before
and after.

Finished when `bun run check` is green, a frame of CILIA over sixty cards is
measurably and largely faster, the DOM ruler is gone or called once instead of
a hundred times, and the picture hashes identically to before.

No `Check:` trailer if the hashes match — nothing a person can see has changed.

Model `sonnet`, effort `think hard`. Read `cilia.ts`'s `onFrame`, `contourAt`
and `pointsAt` first, then `s14`'s branch if it has landed.
