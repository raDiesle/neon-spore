...
step 7's trailer. Do not "fix" any of them, and do not add a second record so
...
Trailers. Do **not** write a `Check:` for how the bulb reads on the field. That
is what the vote was: at true size, at tempo, beside the thing it replaces, and
a trailer would put a settled thing back on a list whose only value is that
everything on it is real. Do write exactly one `Check:` for each reader step 2
turned up that the vote did not put on either phone — for this record that is
the title screen, which draws BULB's contour itself in
`apps/game/src/menu-view.ts` and was never on screen:

        Check: the title screen's bulb still reads at six lobes — `?menu=1`

And if `bun run checks` lists an outstanding check naming this slot (`versus
creature:bulb`, written when the candidates landed), that check is now settled:
record it PASS with the note `voted bulb-deep`.
...
and the deleted directories, and writes no `Check:` at all. Step 0, step 4 and
