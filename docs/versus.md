...
**Does `tools/versus/` resolve `@neon-spore/render` as a plain directory?** There is no `node_modules` in this worktree so I could not test it. Bun hoists workspace links to the root, and `tools/checks`, `tools/burn`, `tools/handoff` and `tools/land` are all plain directories, but none of them imports a workspace package. Lane 1 tries the plain form first; if it does not resolve, it adds a minimal `package.json` there naming render, content and sim — at the cost of a `bun install` in every fresh worktree forever, which is why it is the fallback and not the plan.
