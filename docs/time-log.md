...
| landing | 10 | two `check:fast` runs, the second at eight minutes of wall clock because two lanes were sharing the machine-wide shard cap, plus `index`, `lint`, the commit and the push. The rows sum to more than the lane's twenty-six minutes of wall clock, because that second check ran *under* the writing rather than after it: three lanes at once make the columns overlap, and the rows are what each activity cost rather than a partition of the clock |
