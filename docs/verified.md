...
- `ec1174f` 2026-09-01 PASS — play THE DART. Does the arrow over a hanging dart give player 2 enough to call the side before it goes — and does the leaning body tell player 1 so much on its own that the call stops being worth making?
- `a85510b` 2026-09-01 PASS — on THE CLASP, does the ball read as a lit sphere with a honeycomb on it rather than a hexagon pattern in a circle — and can you still name the colour of the body inside it at arm's length?
- `f22d40b` 2026-09-01 PASS — play THE CLASP. Standing the shield under one — do the arcs off the rim say "these two are lined up" before either of you has pressed anything, and does the body's colour stay readable through the green?
- `b0f379e` 2026-09-01 PASS — on OTHER GRAPHICS → COLLECTED LOOKS, does Green Shield still read at 256 px WebP — or did the re-encode cost the crawling veins anything an eye can find?
- `b32c0a5` 2026-09-01 PASS — play THE THIRD SHOT on a phone. Before a shot is fired, does the plated body read as shielded-but-not-permanently — three cyan splits on a dark plate a size too big — rather than as a rock? And the beat after one column is chipped, is it obvious at a glance which half is the bare body and which is still armour?
- `0e42688` 2026-09-01 PASS — read CLAUDE VS CHATGPT on the NOT BUILT YET sheet — does the task table describe the work you actually do, and is the verdict row for long prose the one you would have written yourself?
- `d8335ed` 2026-09-01 PASS — in the director, does a click on an occupied cell now read as picking it up rather than as a wobble — and is the gold ring on the selected cell still findable when it sits on the marked beat, which is drawn the same weight?
- `2e96dd5` 2026-09-01 PASS — in the director's BRUSH rail, is the DART chip a whole body with its jet, centred like the six around it?
- `ee5fe21` 2026-09-01 PASS — in the director's BRUSH rail, does every thumbnail read as its creature at 34 px — the torch apart from the meteor, the shell apart from the bulb — and does the hover card open where there is room for it?
- `ffd109e` 2026-09-01 PASS — on THE VEIL, does the lightning read as lightning and land hard enough on the beat to count by?
- `a3b70d5` 2026-09-01 PASS — does the veil read as a thundercloud at the size it draws, and does its lightning land hard enough on the beat to count by?
- `bb86e0b` 2026-09-01 PASS — on THE VEIL, side by side — can player 1 read the body through the cloud and the beats left off the ring, while player 2 sees nothing but weather and a question mark?
- `e0bdfd3` 2026-09-01 PASS — the brush thumbnails (the lure's ring and label, the shell's one-plate crop, the shared amber shape mend/purge/ward all draw) want a look on a real screen, not just the automated screenshot this was checked with.
- `06f8739` 2026-09-01 PASS — in the director, select a rock and set it to 2×2 — does it read as one big rock filling a 2x2 square rather than as a normal meteor drawn larger, and is the radar blip over it wide enough to say two columns before it arrives?
- `37dc39c` 2026-09-01 PASS — play THE DART on a phone as player 2. While a dart is still flying, do the arrow, the dotted legs and the hole ahead of it give you enough to call the column before it lands — and at the moment it lands, does the body read as settling into the hole rather than as passing a mark that then jumps?
- `6f79679` 2026-09-01 PASS — does the director still feel permanent while you are working in it — leave a tab open, go away for half an hour, come back and edit?
- `3ef460d` 2026-09-01 PASS — the cannon:shot streak freeze (0.48s) was tuned by eye against a `bun run shot` capture, not watched live at tempo on a phone
- `c240940` 2026-09-01 PASS — on THE VEIL at tempo, does the siren read as "talk" from the corner without pulling the eye off the field, and do the rim bolts still land hard enough on the beat to count by?
- `440d14d` 2026-09-01 PASS — in the director's BRUSH & MAP column, does the accordion read at a glance with every category open, and is 250px the right width for the palette beside the map?
- `56107e8` 2026-09-01 PASS — on THE VEIL at tempo, can each player tell in one glance whether the block on the left or the right is theirs, and does the eye on the strip read as an eye rather than as a blip?
