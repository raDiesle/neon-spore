...
## 2026-09-23 · 1824b62d — THE UNDERTOW's lobes are wet slime pushed up through the plating

Each lobe was a grey fill with a glowing line drawn round it. Now it is wet flesh, lit on its shoulder from above and on its foot by the breach's violet, with a fold where the plate squeezed it and a film of gloss. A tall lobe carries the beam's cyan and red lit inside its wall, over the same thirds as before. The body's last pass is made the same way, in the ship's own deep.
...
## 2026-09-21 · 972960ff — `Where: cloud` comes off; a local session may take anything

The owner, 21 September 2026: *"please remove cloud only. all cloud only also local can and should take"*. Ten entries carried the line and a local session was refused every one of them by name.
...
## 2026-09-20 · a3d1d846 — THE FLEET's five wound-rings, watched through real events not fabricated state

A ring is a transient effect that only exists for the tick a real SimEvent fires, so no --boss-json write to world.boss could reach one. Scripted a real hit (aim four times, then salvo) to get a genuine fleetFlood, and let its own ten-beat window run out into a genuine fleetPlug, then sampled four frames across each ring's life. Flood grows outward from the holed square; plug starts wide and collapses inward — the two directions this boss's six kinds split into, both matching fleetRingRadius's own tested arithmetic and drawn with the same drawThrownRing four other shipped bosses already use.
...
## 2026-09-19 · 4674a3bb — THE LEAD's still is a handle, and her thumb buys the beam its fill
...
## 2026-09-19 · c9be481b — Give the line ceiling room by moving THE ANTIPHON to the second silent-boss page

## 2026-09-19 · 8bf6428c — Give a film's SceneAct a world column mapCol cannot reach
...
## 2026-09-19 · 5d053b3f — Record the owner's answers to ten of the eleven queued asks

## 2026-09-19 · 1faae8d0 — Give THE TASTER and THE SINEW their own page, without reordering the game
...
## 2026-09-19 · a50ebc91 — A queue claim names a branch the lane is not on
...
Under `grip` the heart's room is a handle for player 2 alone: a ring on it that breathes until her thumb lands and fills once it has, PULL over it, and round it on both screens the dial of `mazeGripBeats` running out. The shot is held in the middle and the muscle is carried down under it by the pull, veins stretching from the wall; the pilot's string keeps its handle as the brace, with HOLD over it. `mazeGrip` throws a ring off the heart (`effects.boss.maze`). Two rows on the director's CONTROLS tab, the string's moved out beside the heart's. A look with no shipped alternative.
...
## 2026-09-18 · d9ac58d1 — Mark "BULB QUEEN changes state more than once, and asks for more than one gesture" done

Both halves landed: the simulation in 121cc246, the look in fb3c2384.
...
## 2026-09-17 · b54c15b6 — THE LEAD's rehearsal: a shot at where it is, four at where it will be by `atBoss`, the run's litter warded, and the beam standing in the pass

Eight pages over 1500 ticks in `content/src/scenes/the-lead.ts`, the first film whose `atBoss` strip is a prediction: `sim/boss-answer.ts` answers THE LEAD with `leadLead`, the column the body will be in two beats on — one walk for the bolt's climb up the field and one for its beat in the air — and nothing while it stands dead still or passes. `lead.ts` gains `leadAhead`, the walk that `leadAim`, `leadHeading` and the new line all read. The mistake comes first: a shot at where the body stands, judged two beats later against where it is, a miss and the double back. Then the walk's lead of two, the run's of four and the wall turning inside the sum, the two torches warded where they land, the last segment stopping the body at the right wall, and a colour held three beats early so the beam is standing when the pass comes through at three a beat. The run's rock stands in its column for the fourteen beats of its fall, so the presses are laid around the rocks and warded after the body is down. The wave's guide says `scene: "theLead"`; the prose list drops to fifteen; `scene-films.test.ts` holds the sequence; §3.2 and §11.29 say the film; the queue's gauge-anchor item names the scene. Time: `docs/time-log.md`.
...
## 2026-09-17 · 63f9e730 — THE SCUTTLE, the simulation: a frame that throws itself at you a part at a time, and each part is a window

Lane one of §15 THE SCUTTLE off the choreographed-boss list. A frame of twenty-one sockets hangs over the top of the field, every socket holding a part — a rock, a body in one of the two colours, or a pod — and after four beats of looking a part comes loose on a cadence, hangs for three beats and is thrown down its column as the ordinary arrival it is, while the next comes loose on the same beat. A bolt out of the top in the live part's column and its colour takes it off the frame without a throw, and the next comes loose a beat later; the other colour is rebuffed. From twelve left two come loose a cycle with one live, from seven the cadence is two beats and the live part crosses the field, a thrown pod taken slows every cadence by a beat, and with one part left the frame winds up under THE SLOW for the lance's fill and a beat — only the beam standing in that column ends it, and the last part thrown goes through the hull and loses the wave. It fills its own wave: wave 80, act page `act-7e.ts`, no entries. Ten events with a sound each, the director's sheet, fourteen tests, `docs/spec/bosses.md` §11.30 with every departure argued. The look lane owes the picture and the split — the count on the pilot's screen, the live part on the navigator's — and every event is listed silent until it draws them.

## 2026-09-17 · 027f6160 — Cut the director's two choreographed tables and the sim's boss installer along the seams the tree already had, before the next boss lands on a wall

THE LEAD's landing found `ship-fields-choreo.ts` and `ship-notes-choreo.ts` at 240 lines each and `sim/wave-boss.ts` at 250 exactly, and the next boss brings a dozen fields, a paragraph and a branch. The two tables get a second page from THE LEDGER on (`ship-fields-choreo-b.ts`, `ship-notes-choreo-b.ts`), spread in place so `ship-fields-round.ts`, `ship-notes-round.ts` and their exhaustiveness checks are untouched. The choreographed bosses' install branches move to `wave-boss-clocks.ts` behind an `isClockEntry` guard over a `CLOCK_KINDS` list — `boss-draw-clocks.ts`'s shape made for entries — so the rounds, the pile, the vane, the warden and the queen stay where they were and keep their narrowed types. Every branch still says what its boss leaves on the field. A refactor, not a look: no frame changes.

## 2026-09-17 · 633dee0e — THE LEAD, the look: a ridge above the field, a stalk of beads that stands at its column on one screen and leans in the middle of the other

A look with no shipped alternative. A long rock ridge hangs above row 0 the whole width of the field and the body is a stalk of beads standing out of it, a bead a segment, a pale organ at the tip — its silhouette is its health. The split is the encounter: on the navigator's screen the stalk stands at the body's own column on a mound, with the target lock closed on that column, and never tilts; on the pilot's it stands in the middle column on a sill, in a readout's tone, and leans the way the body goes — the lean being all he is given. A flight is a white bolt over its column on both screens; still, the stalk is grey and upright; on the pass it lies over the way it lunges; down, it is gone and the ridge fades over `leadOutBeats`. The fourteen events are one family read above the loop (`lead-fx.ts`): the lean rides a spring, a doubling back whips it only where the lean is shown, a hit sends the tip bead tumbling, and every burst is placed by what that screen is allowed to know. Eighteen frame tests prove the picture on all three screens and the split both ways; §11.29 gets *The look* and loses *What is not built*.
...
## 2026-09-17 · 950038fb — CONTINUE comes off the PLAY page; the room screen's READY holds mend a parted or quit run

A look the owner asked for by name, 17 September 2026 — the first exemption under *A look is offered, never replaced*. The PLAY page is the partners and NEW GAME, nothing under it. The row's three answers each have a home that is not a row: a wave's START is the two READY holds on the room screen; a menu opened over a running field closes on its own chip; and a run that has parted, or been quit in a room, is mended on the room screen by the same two holds — it opens itself on a parting like any other fault, the shell takes the menu down under it, and the circles are holdable again while the worlds have parted or a QUIT stands (`join-room.ts` `mayHold`). A QUIT with no room still goes to PLAY. Nothing reaches the wire: the room already restarted a stamped run on two `ready` presses (`room-start.ts`, 13 September) and `ready()` still sends after a parting, so the net-change skill and the relay check were not owed. The whole queue entry is done with this; a check that went red only while another session's check shared the cores is queued.

## 2026-09-17 · c6b0a3b4 — THE SINEW, the simulation: two pulls add into one sum, and the pair says how hard

The boss that asks how hard rather than when (docs/spec/bosses-choreographed.md §8), built as a field boss with no body in its first lane. A mass hangs over the middle column on six fibres with a handle either side — `sinewLeft` is player 1's, `sinewRight` player 2's, two new `DragTarget`s whose `fromYMilli` is the pull — and the two pulls add into one sum less the rope's slack. The fibre being held parts when the sum sits inside a zone for four beats on end: the zone is rolled fresh from the rng for every fibre and narrows by a step each, player 1 is shown where it is and player 2 what the sum is, and neither the other. A sum over the zone's top snaps: both hands thrown off for two beats and a rock out of the mass, three on the last fibre. From the fourth fibre gone the rope creeps slack under a held hand until both come off. Each part sinks the mass a row and opens THE SLOW. When the last parts the mass falls over four beats, both handles swayed the same way walk it a column a beat, and three columns from the middle it lands clear — fewer and it lands on the hull, a heavy breach that fails the wave. The cannon cannot hurt it. Wave 76, THE SINEW, carries its own arrivals so that letting go is a decision. Written up as `docs/spec/bosses.md` §11.26 with nine departures from the design argued by name; the look is the next lane, and the thirteen events are silent until it draws them. The wire's `DRAG_TARGETS` now carries the two handles — it drops a target it does not list without a word, and the skill's table names that file now. Wave 76 went into the baseline unweighed. The fourth boss family put `render/src/effects-spark-silent.ts` over 250 lines, so the bosses' silent rows are next door in `effects-spark-silent-boss.ts`.
...
## 2026-09-17 · 2a78763c — THE CURTAIN, the look: a violet membrane both hands carry, and the core seen through it by one seat

A look with no shipped alternative. A translucent membrane in the hull's violet hung from a rail across seven columns, a fold swaying in every column and a scalloped hem with a grey lobe weighing each scallop — lifted where a lobe has come off. A shove carries it the way every glide is carried and the hem trails the rail through the beat. The core is drawn first and the fabric over it, which is the whole of the occlusion: covered, a halo and a dimmed disc in its colour on the navigator's screen and nothing on the pilot's; bare, a five-lobed blob rimmed in its colour on both; naked, the same blob pulsing; going out, fading over `curtainOutBeats`. The pilot's screen lights the soft lobes in the hull's rim (`showsCurtainSoft`, `showsCurtainShadow`). The hand ring is drawn over the sheet, THE CAIRN's reason, closing on the part of the fabric that is on the field. The ten events are one family read above the loop (`curtain-fx.ts`), and the tear keeps the sheet falling and crumpling out of the picture over four beats — the one transient, cleared on restart. `curtain-frame.test.ts` proves every state on all three screens and the split both ways; §11.24 gets *The look*.
...
## 2026-09-16 · 77fbb990 — The repository walk reads its eighteen hundred files sixty-four at a time

`tools/test/tree-walk.test.ts` awaited one `Bun.file().text()` per file inside a `for`, over every `.ts` under packages, apps and tools. Alone that is 350 ms; inside `bun run check`, with thirteen shards reading the one disk, it crossed the 5000 ms cap and took a landing red — and the same command was green on the next run with nothing changed, which is the worst shape a red check has, because it teaches the next session to re-run rather than to read. It now reads in chunks of sixty-four and finishes in 111 ms.
...
## 2026-09-16 · 5c671f4c — Five looks for the tutorial's chrome in VERSUS, and a seam they patch

The owner was not happy with the tutorial screen's rounded buttons and text boxes, and asked for several whole new variants that look like the game's own ship and controls, say NEXT in a word, arrange the header and the buttons the way mobile games do, and leave no doubt that it is a tutorial. The band across the top, the bar of buttons and the box of words now go through one seam, `GUIDE_LOOK` (`guide-look.ts`), whose defaults are the shipped functions, and `navHit` reads the buttons from it so a candidate may put them anywhere on the stage. A pose, GUIDE · A PAGE OF FILM, holds player 1's phone on the second page of the first tutorial, and `guide:chrome` has five candidates on it, every one with BACK and NEXT written on them and the header twice the size: spotlight (the field dimmed to the subject, a speech bubble), ribbon (hazard stripes across the top and the bar), console (a bezel top and foot, one NEXT the width of the phone, a dashed ring), rail (BACK and NEXT grown out of the side walls at thumb height, the header at the foot) and coach (a mint coach body whose bubble says TUTORIAL at thirty points, the field dark, NEXT as wide as the phone and BACK small under it — the shape Clash Royale's king and Training Camp take, with the owner's mid-turn asks of more dark, bigger buttons and a colour neither ship wears). A pose may now name where the same screen can be seen in other games (`Pose.elsewhere`), printed on the VERSUS row as links that open in a new tab; the guide pose names three pages of Clash Royale's.
...
## 2026-09-15 · 628eeba4 — Everything on a harpooned control is drawn where that control is drawn

Taking the first picture of a placed leech found three things, and none of them was reachable from a test.
...
## 2026-09-13 · b2d6f3fd — The hand's ring is drawn on THE WELL's cannon

`drawShip` left for the well's own pass before `drawShipHand`, so a thumb the well answered on its cannon (`touch-well.ts`) had nothing drawn under it — the one place a finger's own mark would tell the pilot which hour he has hold of was dark. `drawShipHand` takes where the ring goes and which way is up there (`PlaceHand`), the flat hull's being the grab circles it always used, and the well hands in `wellHandPlace`: its grab circles at the world's column, turned to the lobe's hour, so the cup sits over the swelling's outward face and the slide arrows point along the ring. A fix to something wrong rather than unlovely, the third exemption: the control is answered where nothing showed it.
...
## 2026-09-13 · 87968001 — A guide capture counts one film tick per --ticks, not two

freezeClocks in tools/frames/page.ts wrapped window.neonSpore.paint to advance the frozen clock a sixtieth per call, and dropped the dt the capture passes for a rehearsal. A film is run off that dt — one tick per paint(1 / tickHz) — so every count of an --opening guide capture was a sixtieth of a second and two film ticks, and --ticks 80 on a page photographed its tick 160. The wrapper passes dt through now.
...
## 2026-09-12 · ecaec07d — Seven rules the simulation owned were written out again in render, content, audio and the director

The copies table had stood at 46 rows since it was built, and nothing had swept for the next re-derived rule. One grep per config field that a second package reads found seven: the tempo as seconds (`60 / bpm`, fifteen copies across four packages, three of them a module constant each called `BEAT_SECONDS`), the tempo as ticks (`guide-scene.ts` wrote `tickHz * 60 / bpm` twice on one line, past the check that the quotient is whole), the runaway cannon's firing beat (the emitter had `stepMalfunction`'s line again), SNAKE's trigger rest (the panel greyed the slab by its own copy of the comparison), the pinball lane's floor (content in tiles, sim in thousandths), THE GAUGE's clock (the bar under the dial, where THE FLEET's already asks `fleetBeatsLeft`) and the countdown's slots (four dials clamping `countdownBeats` on their own). Each is now one function in `sim` — `beatSeconds`, `faultFiresThisBeat`, `snakeResting`, `pinLaneFloorMilli`, `gaugeBeatsLeft`, `countdownSlots`, and `ticksPerBeat` called — with its callers converted and a row in `copies-table.ts`, so the table is 53 rows and the next copy of any of them is a red check. Nothing drawn changed.
...
## 2026-09-12 · 24a27f65 — The NOT BUILT YET boss page says what each idea asks of the pair's hands, and nothing else

At the owner's ask, `docs/spec/ideas.md`'s Bosses and Rounds — the two groups the director's boss page is built from — are cut to what fits the game as it stands: four bosses (THE VANE left, being built) and seven rounds, each a few lines on the gesture or the information split it is made of and what each seat's screen shows, with the silhouette talk gone to the SHAPES tab where the pictures are. Eight rounds went — the ones counted on the beat or a reflex game (THE DUET, THE CRANK, THE DIVIDE, THE FLOOR, THE BELT), a mechanism without a round (THE REPRISE, THE EDGE) and a second *you read, I hold* (THE THROTTLE); their arguments stay in `party-games.md` and `transfers-hazelight.md`, and the Rounds preamble says so. The bold names stay as the handles the director parses and the drafts point at.
...
## 2026-09-12 · 03456210 — THE CHOKE is the third fault, not a body: the steer fault walks the cannon for the whole wave, no brush

The owner asked for THE CHOKE — built yesterday as a strand that fell, took the cannon and was tapped off — to be *the same kind of control set modifier* as THE JAM and THE COIL, *no brush*. So `Malfunction` has a third kind, `steer`, authored on the wave like the other two and on from its first beat to its last: the emitter hangs at the top of the field with its beam on player 1's cannon strip and on the muzzle, the strip goes dead, and the cannon walks a column every `chokeSweepBeats` beats from the middle, wall to wall and back — a stateless triangle walk off `waveBeat` (`steerCol`), since every wave starts with the cannon in the middle. The trigger stays with player 2, who fires on the beat the cannon passes under a body; only player 1 is shown the light along the hull toward the column it steps to next, and calling that column ahead is the wave. The loops round the swelling and round the strip's node stay as the picture of the grip. The `"choke"` creature kind, its silhouette, three events, crawl, strand, drag target, brush, field-control row, pose, sound binding and eight tests are gone; the tap-off is written up on the NOT BUILT YET page under Mechanics, and the roster removal and its bar are decision #31.

## 2026-09-12 · a576faa2 — THE LIMPET and THE LEECH: a body that takes a control and goes off if the control stands still

The second piece of the malfunction task, and the rule it was set under: a malfunction has an indication and a visible cause. THE LIMPET falls straight down one lane, cannot be shot, is not stopped by the shield, and takes hold of the plate where it lands; THE LEECH is its twin on the cannon. From the beat one has its control, every beat the control is found in the column it stood in a beat before is a beat of the fuse — `limpetStillBeats` of them and it goes off, a heavy hit on the hull at the control's column, the wave lost — and a beat it is found elsewhere puts the fuse back and is one of the `limpetShakeMoves` that shake it off. Only the seat without the control is shown the fuse, as a row of lights over the body going out one a beat; the seat that can move sees the body lift and its hooks open as the moves mount, and nothing else. The word *move* is what joins them.
...
## 2026-09-12 · 8d52fa9c — A guided wave carries its lesson and nothing else

The owner's rule for the wave the pair plays, not the film: a wave that teaches something has only the enemies the lesson needs and no other kind. Rocks and every special kind that are not the lesson go; plain slicks stay only where the lesson needs a target, at the fewest that make the point. Thirty-two guided waves are trimmed, 90 entries out — a stray rock was on most of them, and THE CLASP, THE VEIL, THE WISP, THE GHOST and their like carried two or three slicks beside the thing they were about. What stays besides the lesson's own kind: the real bodies a lure is told apart from (THE LURE, THE JAM), the bodies a purge clears, the targets a jammed or walking gun is pointed at (THE JAM, THE CHOKE), the lane a crossing rock takes (THE CROSSING), the two bodies THE ECHO is taken ahead of, the slick under each gum, the rocks and slicks the arm folds (THE VANE), and one rock in the lane between plate and dome (THE COIL). The subject's own count is untouched.

## 2026-09-12 · 55cc535f — The room tests wait for the message they read, and `check:fast` reaches them

Two landings went red on `apps/server/test/room.test.ts` for lanes that had not touched it, each a full check to find out. Why it races under a full shard run and never alone: eight `bun test` processes on a CPU capped at half starve workerd and the test process both, so a 60 ms quiet interval is no longer a round trip; a wait for "one more `ready`" is satisfied by the one a join sends to a full room, not the one the press sends; and a 150 ms silence window is crossed by the wall clock between two lines of a test, so the room hangs up a seat the test believes is there. Alone, the same lines are microseconds apart. Every bare `settle()` before a read is gone: `settle` takes a predicate on the last message of a kind (`settle("welcome", (w) => w.startMs > 0)`), which cannot be off by one, and `caughtUp()` fences a line that asserts nothing arrived with a ping the room answers only after everything sent ahead of it. The shortened windows are one 600 ms figure, and the two tests that wait a window out arrive until the room says it has passed. The shared workerd's boot is paid in a `beforeAll` against the budget written for it, not by whichever test comes first. Proved on this machine with `bun run tools/check/shard.ts room` five times in a row, 27 pass each, no red.
...
## 2026-09-11 · 0703fb29 — creature:volley decided: EMBER burns through the seams in the game, PITTED and the painted seams kept on the LIBRARY

The owner: "Apply to game CREATURE:VOLLEY · EMBER. keep CREATURE:VOLLEY · PITTED for 'Shapes' page, i like this rotation - maybe to apply for 'Meteors'". A look the owner asked for by name. EMBER's seams move into packages/render as volley-ember.ts and VOLLEY_LOOK.seams points at them: each seam is the body inside burning through the joint, scorched along both sides with a warm white core, burning wider with every ward. PITTED moves in beside it as volley-pitted.ts for the SHAPES page's LIBRARY only, with the owner's note about the meteors on its card; the painted seams the game wore until today stay as shippedSeams and get a card of their own. The LIBRARY gets a volley stage: the game's own drawVolleyShell over the game's own drawLiving, walking the plate count down three, two, one and round again on the page's beat, with both record fields swapped for the length of a card and put back. GROOVE goes with the slot.
...
## 2026-09-10 · 54997ed0 — Three shells for THE VOLLEY and three riders for THE VEER, offered in VERSUS

`creature:volley` and `creature:veer` open with three answers each beside what ships, on two records cut out of the paint: `volley-look.ts` names the shell's stone (inside the ward's clip) and its seams (drawn whole over it), and `veer-look.ts` names the rider on the rock, with the shipped paint of each moved to `volley-stone.ts` and `veer-rider.ts` so nothing draws differently. For the volley: GROOVE cuts the four seams into the stone as trenches with a lit lip and a shadow that swap sides as the ball rolls; PITTED gives the stone a meteor's own pits, placed by longitude and latitude and turned by the roll so they come round; EMBER makes the seams the body inside burning through the joints, scorched along both sides and burning wider with every ward. For the veer: SOLID lights the same clown as a solid under the key with a contact shadow on the stone; HUNCHED gives it a body hugging the crown with arms down the flanks; JESTER swaps the cone for a two-horned hood with bells that swing with the sway and whip on the brace. Two poses: `VOLLEY · WARDED THREE TIMES` keeps a hand on the shield so every ward happens on the page, and `VEER · RIDING DOWN` follows the rider through its lane changes. Offered, not replaced: the field draws exactly what it drew.
...
## 2026-09-10 · 6f3b3362 — A tile crop follows the body as it is drawn, not the tile it is stepping to

`firstOfKind` answered a `tile` crop with the body's target tile, and the renderer draws the body gliding there from where it was over the beat, so the window was a beat ahead of the picture: within a tile for a body that moves a tile a beat, and for a dart two columns, a carom three or a chute four rows, outside its own window for most of every beat. It answers with `drawnCol` and `drawnRow` at the world's own beat phase now — the interpolation the renderer uses, exported from render for it — so the pair's window glides with the body instead of jumping ahead of it. `versus-crop-follow.test.ts` gains the check the old one could not make, because it measured the body by `at()` itself: at the middle of a beat, every tile-cropped pose the pair can open is centred no farther from its nearest body as drawn than from that body's target tile. Nine poses fail it without the fix, the dart and the chute among them; a fixture that never moves, and the queen's pose that centres a tile below her on purpose, pass either way. The gallery's still cards, several of which centre a fixed tile, are not held to it: a still is never stepped.

## 2026-09-10 · 377b16b2 — A queue claim writes its `Taken:` line onto `main` in a clone that has nothing checked out on it

`bun run queue take` in a session started from a phone answered `⚑ left alone — nothing has main checked out` and reported the item ongoing anyway: the branch was made, which no other clone can see, and the line was not, which is the half written for exactly those clones. `onTrunk` now writes the commit onto the ref directly when no worktree holds the trunk — the branch's tree read into an index of its own, the one file replaced, `commit-tree` and `update-ref` guarded by the old tip — so the lane the session stands on and its working tree are never touched, and the push goes as it does everywhere else. A clone without a `main` at all is refused rather than half-claimed. `ref-commit.test.ts` proves it against a repository shaped like a clone.

## 2026-09-10 · 1f78f828 — Three recoils that are not cages, offered in VERSUS

The owner asked on 10 September 2026 for answers to `creature:recoil` that are completely different from a frame — anything, so long as what sits inside keeps the cannon's colour. The record already guarantees that half: the body is drawn underneath in its own colour and every colour the cage is handed is stained to it, so these three throw the frame away and keep the count. FOAM sticks one bubble per bounce to the body's outside, each a translucent ball with a rim that catches the far light and a crescent toward the key, wobbling and swelling with the strain; a spent bounce is the burst ring of one. MOONS puts one moon per bounce on a ring tilted toward the viewer and turning on the clock, placed by `pin` and projected by `facet` so a moon comes round from behind the body large and lit and shrinks away dim; a spent one is a cinder still on its orbit. CALYX fans one leaf per bounce out and down from under the body, each a cupped blade with a midrib lit by where it faces the key, so the body sits in a calyx like a berry; a spent one is wilted, half its length and scorched. All three take the slot's existing pose, one bounce spent. Nothing the game draws changed.
...
## 2026-09-10 · 1068f4ba — Three sheds and three bodies for THE RIND, three armours for THE LID, in VERSUS

`creature:rind`, `rind:body` and `creature:lid` open with three answers each beside what ships. The rind's shed — the half-second in which a layer comes off — is now a record, `RIND_LOOK.shed`, with the shipped picture in `rind-skin.ts`: POD splits the skin down its meridian into two half-shells that swing open on the limbs and turn past edge-on to show their insides; FLAKES breaks it into a dozen thick pieces tumbling under one key light; SLOUGH drops it whole like a sock, stretching and gathering shut at the mouth. The owner asked the same day for a rind that is not a slick, so the record also carries `body`, the silhouette a rind wears per layer it has left, and `rind:body` offers three off the shapes page: TOOTHED, THE RIND card's own rim of sharpened teeth that wear down with the layers; FACET, the MOULT card's faceted shell, twelve sides with both layers on and eight with one; BURR, the studded form asked for fewer, shorter knobs per layer lost. Colour, size and interior are the creature's own, and the husk the shed throws is whichever contour it wore. THE LID's plates are `LID_LOOK.plates` (`lid-plates.ts` holds the shipped ones): BEVEL gives the sliding plates a curved face and a wall at each inner edge, lit oppositely under the one light; ROLL winds each plate into a cylinder that fattens as the eye opens; IRIS is six overlapping leaves closing to a point on the rule's own gap.

## 2026-09-10 · 26335afd — `versus:shot` comes back in seconds at any scale, and the pair on a cadenced pose runs at full rate

Two faults wore one coat. The seat probe read its two canvases back with `getImageData` up to forty-eight times a candidate without declaring it, and past some count Chrome did more than move those two off the GPU: every phone-sized canvas the page made afterwards was rasterised in software, with a pixel readback for each sprite it drew. A pair on any cadenced pose — echo, dart, magnet, meteor — ran at three frames a second at scale 1 and under one at scale 6, before the camera was involved at all; a pose with no cadence took twenty-four samples and stayed under the threshold, which is why the ghost worked. `Canvas2DRenderer` now takes `{ readback: true }` and the probe says so, and the same page runs at sixty. Then `versus-shot.ts` started the director through `cmd.exe` and its `kill()` reached only the shell: the director lived on, holding the pipe, until its own idle exit two and a half minutes later — every shot on Windows paid that, and the lane that filed the hang read the wait as part of it. It is `Bun.spawn` now, the way `serve.ts` does it. `bun run versus:shot creature:echo cleft --freeze 1.1 --only candidate --scale 6` was a timeout, then 2 m 31 s, and is 9 s with the echo in it. The probe's raster is also deterministic where the GPU's was not, so a body look the same on both seats is one screen rather than two; the hull skin, which really differs by seat, still gets both.

## 2026-09-10 · acd65b8f — The scratch-tree wave test gets the minute its checkout and install can take

`waveNamesAt` makes a scratch worktree at a commit and runs `bun install` in it, and the test that proves it ran under bun's five-second default: under eight shards on a loaded machine that timed out on four landings in one afternoon, and every rerun was green. It has sixty seconds now, which is the work it does rather than a guess, and the queue entry that named it is gone.

## 2026-09-10 · 458c534e — A red check under `land` names the tests that failed and keeps its output

`bun run land` printed the last twenty-five lines of a red `bun run check`, and with eight shards the name of the failing test sat well above them: three landings on 10 September 2026 went red on the same flake and none could say which test it was, so it went into the queue nameless twice. `red-check.ts` now prints every `(fail)`, red-shard and timeout line first, the tail after, and writes the whole output to a file under the temp directory that the last line names. The queue entry for the flake — `wave.test.ts` reading WAVES at HEAD through git — no longer asks for this half.

## 2026-09-10 · 3fd34069 — Three outlines for THE MOUNT, offered in VERSUS

The owner answered on 10 September 2026 that a mount should read as a mount at a glance, reversing the note the recoil lane had just written into DECIDED.md (withdrawn here). `creature:mount` opens with three answers beside the slick or bulb a mount has always been drawn as, on a seam cut for it: `mount-look.ts` holds the one thing a mount may differ in — the contour it is drawn with, `undefined` as it ships — and `creature-body.ts` gives the mount a row of its own that hands that answer to `drawLivingBody` the way THE BEATBOX's arms go in, so the colour, the skin, the interior and the sway stay the living draw's. All three outlines come from the shape sheet, turned to face the wheel and read once per rim bearing: TAPROOT is the sheet's rooted bulb with its roots reaching in toward the hub; RASP is THE RASP's ring of twenty spines, given the crown `studded` offers so the longer ones face out; PENDANT is the `sac` form tuned to a drop, narrow at the rim and heavy toward the hub. The slot takes `GYRE · TURNING`. Nothing the game draws changed.

## 2026-09-10 · ab58742f — Three cages for THE RECOIL, offered in VERSUS; THE MOUNT has nothing of its own to offer

`creature:recoil` opens with three answers beside what ships, on a record cut out of `recoil.ts` for the frame — `recoil-look.ts`, the shipped springs and hoop unchanged, the count, the colours, the breath and the burn still worked out by the caller so no look can change what the ribs say. TUBE keeps the shipped folds (`ribPath`, cut out of `drawRib` for it) and the hoop's arcs and gaps, and lays every stroke three times — dark, then lit and shifted toward the key, then a thread of light on the crest — the hoop lit round its ring by its own normal and the bolts as balls. GLOBE makes the cage a wire ball: each rib a whole meridian through the poles, pinned and projected by `facet`, its near half lit over the body and its far half dim behind it, the hoop a tilted equator, the ball turning so every rib swings from a line at the limb to a curve at the front. SPRUNG winds each rib into a coil, every turn a loop at a slant with its near half bright over its far half dark, on the shipped hoop. The slot takes a pose of its own, `RECOIL · ONE BOUNCE SPENT`, in `poses-cage.ts`, because a cage with nothing wrong with it is three springs that all look alike.

## 2026-09-10 · 3d08542a — Three shells for THE QUEEN, offered in VERSUS

`creature:queen` opens with three answers beside what ships, on a record cut out of `queen.ts` for her armour — `queen-look.ts`, the shipped gradient and stroke unchanged, the contour still built by the caller so no look can change her silhouette. FACET makes the shell a cut stone: a raised table and a crown of thirty-two planes down to the sixteen corners of her contour, each lit by its own normal through `surfaceLit`, the table breathing so the crown's light tips and turns under a light that never moves. CARAPACE makes it a domed back under the shipped key ramp, rounded to a cool rim with a wet shoulder and bounced light along the far edge, with thirty spiracles pinned round her long axis that rise over the crest and sink under her lower edge as she heaves — the reveal a pose cannot fake. SCUTES makes it seven plates lapped from the wings in, each cut from her own contour and filled as a ridge, the lapping edges throwing shadows and wearing bevels, breathing apart with a tilt running wing to wing. The slot takes `QUEEN · SHUT`, which now says where to look, and the queue entry that asked for it is gone. Nothing the game draws changed.

## 2026-09-10 · fcbfe8b6 — Three turns for THE THROB and three skins for THE CRAWLER, offered in VERSUS

`creature:throb` and `creature:crawler` open again with three answers each beside what ships, both about whether a motion reads as a solid thing. The throb's far half: PORES, seven marks pinned on the far hemisphere and carried round by the turn, each crossing the middle fast and crawling at the limb; GIRDLE, the cut between the colours as a raised welt with a lit flank and a shadow flank over a far half lit as a dome; CAP, the far colour as a shell standing proud of the body, its cut edge a wall that swells from nothing to its full thickness and back twice a turn. The crawler's rings: SETAE, a girdle of bristles placed on every ring and swept back by the squeeze; WRINKLE, meridians of fold that come up as a ring contracts and smooth away as it slackens; GUT, a dark organ seen through translucent skin, pushed to the tail by the squeeze and swinging back a beat behind. `throb.ts` exports its seam so a candidate calls it rather than copying it. Both slots take the pose their first slot was judged on, and the queue entry that asked for them is gone.
...
## 2026-09-10 · 72c21990 — Three interiors for THE GHOST and three seams for THE ECHO, offered in VERSUS

`creature:ghost` opens with three answers beside the shipped nebula, on a field the record grew for it — the interior under the camouflage, which shipped as one flat gradient the same from every side. LANTERN lights the dome as a ball under the key with a band of bounced light on its far rim and the nebula's heart pinned to the surface and carried round by the camouflage's own turn; HOLLOW makes it a glass bell lit on the inside of its far wall, dark where a solid would be bright, with a heart hanging inside; SWARM replaces the nebula with fourteen motes streaming round a ball inside the contour, each going round the back and out the other side. `creature:echo` opens on a record cut out of `echo.ts` for the furrow — `echo-look.ts`, the shipped line unchanged — with CLEFT, a groove with a lit wall and a shadowed lip; WAIST, the seam as a meridian on a ball that pinches as the parting comes; and BUDS, two lit cores under one skin drawn apart along the axis. The echo gets a pose of its own, run from the arrival to the second division, and both slots are named in `versus-pose.ts`.

## 2026-09-10 · 6408ead9 — The test suite is profiled, and the three files that were a third of it are cut

`bun run test:profile` runs the suite once through bun's JUnit reporter and prints the slowest files with their share of the run, the slowest cases, and a sum per package — the same process and order as `bun run check`, so the figure a file gets is the figure it costs. The first reading said the guess about the render frame tests was wrong in the useful way: one file was thirty percent. The rehearsal walks in `briefing.test.ts` drew a frame per tick, four times over; they now draw one tick in four, each walk taking a different phase, so every tick is still drawn once (88 s to 24 s). `copies.test.ts` ran seventy thousand cases that each re-read their file; it is one case per rule over files read once (20 s to 0.6 s). `tell-frame.test.ts` replayed a seeded game it had already watched. The suite is 292 s to 214 s on this machine, the profile and the rule for a new frame test are in `docs/performance.md`, and the rest of the cut to two minutes is queued file by file with its figures.
...
## 2026-09-10 · aedc6564 — creature:gyre and creature:magnet: three second answers each

The organelle in the middle of THE GYRE's wheel and THE MAGNET's horseshoe each had one look and no second answer. Six candidates now stand beside what ships, none of them on the field. For the gyre: VORTEX draws the fluid as a throat going down, five terraces into shadow with three arms of light spiralling in faster as they go; ORBIT girdles the ball with one tilted band, its far half seen dimly through the mass and its near half drawn over the nucleus, swapping once a turn; HELIX winds a single strand of thirty beads round the inside of the membrane, turning with the wheel and creeping up into a head. For the magnet: ROD gives the horseshoe a round section, a cut face of colour across each pole's end and a plate with a side; ORE makes it dug-up stone, pitted, the plate split into strata, the colour a vein with a bead of light flowing down it to the tip; YAW swings it on its staff so the faces narrow and the sides come into view.
...
## 2026-09-09 · 4f670a39 — A destroy event says what died

`destroy` carried a column, a row and a colour, so the only thing a break could ask was the bestiary's map from ammunition colour to body: red is a slick and cyan a bulb. That is right for an ordinary kill and wrong for every body that is neither — a magnet, a ghost, a lid and a balloon all emit the same plain event, and `effects-break.ts` would have cut their pieces out of a slick's outline at a size where the difference shows.
...
## 2026-09-08 · 9fe0e3d — THE CLAW's arm is wound home on a crank

The panel's press was half a control: REACH sent the arm up a column and it came back on its own with whatever it had closed on, so the pair spent one button and then watched it work. The way back is a gesture now. A fourth button on player 1's half is a winder, and a finger going round and round inside it brings the rope in four tiles a turn — the arm hangs exactly where it stopped for as long as nobody turns it, and the strip stays dead the whole time, so he is winding while she reads the field out to him.
