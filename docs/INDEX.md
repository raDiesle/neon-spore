...
| `packages/sim/src/instar-words.ts` | **The words THE INSTAR's script is written in**: whose thumb, which part, which gesture, which pose |
...
| `packages/content/src/scenes/the-count.ts` | THE COUNT's rehearsal: the shot that is right in every way but its moment |
...
| `packages/content/src/instar-script.ts` | THE INSTAR's script: twenty-eight steps over twenty-one poses, and what the pair does to each |
...
| `packages/render/src/boss-cue-read-zc.ts` | **What THE MANTLE is asking for** — page twenty-nine of the readings |
...
| `packages/render/src/instar-poses-fourth.ts` | **THE INSTAR's fourth act, as figures** |
...
| `packages/hands/src/autopilot-fence.ts` | **THE FENCE, on AUTO**: the shield parked in a gap, or the cannon burns a crack first |
