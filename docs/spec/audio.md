...
| `boss` | the bosses that are built, and the names holding a slot | 87 of 96 |
...
109 of the 280 are `spare`: finished, tested, and nothing plays them. That is
...
`bind-fence.ts`, `bind-baton.ts`, `bind-undertow.ts`, `bind-candle.ts`, `bind-gorge.ts`, `bind-curtain.ts`, `bind-taster.ts`, `bind-warden.ts`,
