...
| `boss` | the bosses that are built, and the names holding a slot | 65 of 74 |
...
109 of the 258 are `spare`: finished, tested, and nothing plays them. That is
...
`bind-fence.ts`, `bind-baton.ts`, `bind-undertow.ts`, `bind-candle.ts`, `bind-gorge.ts`, `bind-warden.ts`,
