...
| `boss` | the bosses that are built, and the names holding a slot | 75 of 84 |
...
109 of the 268 are `spare`: finished, tested, and nothing plays them. That is
...
`bind-fence.ts`, `bind-baton.ts`, `bind-undertow.ts`, `bind-candle.ts`, `bind-gorge.ts`, `bind-curtain.ts`, `bind-warden.ts`,
