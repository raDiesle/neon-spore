...
| `ship` | cannon, shield, maw, and THE GRIP | 12 of 18 |
| `impact` | what a shot does when it arrives | 11 of 14 |
| `hull` | the hull taking it | 6 of 10 |
| `pod` | hanging, loose, taken, lost | 5 of 10 |
| `boss` | the bosses that are built, and the names holding a slot | 15 of 25 |
...
| `creature` | the bestiary, built and unbuilt | 9 of 38 |
...
120 of the 205 are `spare`: finished, tested, and nothing plays them. That is
...
`bind-creatures.ts`, `bind-carom.ts`, `bind-crawler.ts`, `bind-fence.ts`,
`bind-volley.ts`,
