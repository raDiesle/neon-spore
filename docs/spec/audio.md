...
| `boss` | the bosses that are built, and the names holding a slot | 28 of 37 |
...
113 of the 219 are `spare`: finished, tested, and nothing plays them. That is
...
`bind-fleet.ts`, `bind-choir.ts`, `bind-beatbox.ts`, `bind-breach.ts`, `bind-gum.ts`, `bind-choke.ts`, `bind-mirror.ts`, `bind-lookups.ts`, `mixer.ts` and
