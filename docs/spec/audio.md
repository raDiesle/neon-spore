...
| `boss` | the bosses that are built, and the names holding a slot | 103 of 112 |
...
109 of the 296 are `spare`: finished, tested, and nothing plays them. That is
...
`bind-fence.ts`, `bind-baton.ts`, `bind-undertow.ts`, `bind-candle.ts`, `bind-gorge.ts`, `bind-curtain.ts`, `bind-taster.ts`, `bind-sinew.ts`, `bind-warden.ts`,
