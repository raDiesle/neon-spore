...
| `boss` | the bosses that are built, and the names holding a slot | 56 of 65 |
...
109 of the 249 are `spare`: finished, tested, and nothing plays them. That is
...
`bind-fence.ts`, `bind-baton.ts`, `bind-undertow.ts`, `bind-candle.ts`, `bind-warden.ts`,
