...
> order.** A wave with a guide opens on it, and on all but two of them that is
...
### 3.2 The rehearsals — all but two of them
...
**There are eighty-nine of them now**, one per guided wave bar two, and
each is authored in `packages/content/src/scenes/` as a page of choreography
rather than a page of prose. The two that are still the three strings
and the two circles are **THE NETTLE and THE MANTLE** — and
...
**Both of those are films nobody has written, and since THE
