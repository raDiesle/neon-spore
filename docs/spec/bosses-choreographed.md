...
| [§2 THE ORRERY](#2-the-orrery--whether-you-can-agree-on-when) | **simulation built, two lanes to come, 17 September 2026** | `claude/boss-orrery` — the rings landed (wave 70 THE ORRERY, [bosses](bosses.md) §11.21: three orbits anchored so the first alignment is placed rather than hoped for, the shaft, the shot that takes the outermost ring and changes the core's colour, the organs it sheds, the core's own fire and where it may not aim it, THE SLOW over the alignment, the lance that finishes it). Four departures from §2 are argued in §11.21 by name: **the column is never in question** — rings concentric about a core admit exactly one, so the design's *call the column the gap will stand over* is dropped as a second QUEEN — **the orbits are not coprime**, because coprime is the right tool for two cadences and the wrong one for three, **it fills its own wave**, which is the exception THE DIASTOLE's lane allowed for, and **a broken ring sheds three organs rather than eight**. **The hand on the ring is the lane now running**, on `claude/orrery-hand`: a bearing drag on the outermost unbroken ring, `crank`'s thousandths-of-a-turn on the field rather than the panel — the design's step 10, and the only thing that can bring an alignment forward. **One lane left and it is free: the look** (three beaded orbits, the per-seat truth in `render/view-role.ts`, the shaft of light, and a core that is the only coloured thing in the frame) — and it now also owns **the handle the hand takes hold of**, because a hit-test is the ring's drawn circle and nothing in `packages/sim` knows a pixel: the drag lane ships the rule and the wire, and the ring answers a real thumb when the look gives it a radius. Claim it here first |
| [§14 THE CANDLE](#14-the-candle--whether-you-can-act-in-the-dark) | **simulation built, the dark taken, 17 September 2026** | `claude/boss-implementation-e3cfff` — the glow landed (wave 72 THE CANDLE, [bosses](bosses.md) §11.22); the dark is under the same hand now. The two lanes, named before the first was started: **the glow** (the boss entry, its five steps of glow as health, the column it faces, a flash in that column eaten and its glow re-brightened, the last glow, the wave held open until it is out — the simulation, and the receipts) and then **the dark** (the field going black, the muzzle, plate and beam as the only light and each on the screen of the seat that made it, the after-image buffer in `Effects`, a scar glowing, the two black beats before the wave-end light — everything in `render/`). The next free concept this session takes is claimed here again before a line of it is written |
| the other nine | free | — |
