...
| [§3 THE GORGE](#3-the-gorge--what-not-to-do) | **taken, 17 September 2026** | `claude/boss-implementation-e3cfff` — two lanes, named before the first is started: **the sack landed** (`fb63f2ab`, `docs/spec/bosses.md` §11.23, with five departures argued there — the spat bead is a body broken by its own colour, restraint is against the miss and not for the breach, the spit comes from the emptiest intake, the mouth is ended by the beam alone while full, the sink is derived; **the skin is under the same hand now**) (the boss entry, seven intakes each holding a count of beads of one colour, a bullet that meets no creature swallowed into its column's intake, a wrong colour taking a bead back out, an intake full at four and pierced by the fifth or venting a torch down its column after four beats, the sack sinking a row for every four beads it holds, the spat bead as a bolt only the opposite colour out-shoots, the curtain after the third rupture, the beam through the last intake ending it — the simulation, and the receipts) and then **the skin** (the translucent sack breathing on the beat, the beads hanging in it, the pilot's tally under each lobe and the navigator's nearest-full lobe with its colour, the rupture hanging open, the fifty beads leaving at once — everything in `render/`). The next free concept this session takes is claimed here again before a line of it is written |
