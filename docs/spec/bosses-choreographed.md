...
| [§5 THE LEDGER](#5-the-ledger--whose-body-takes-it) | **taken, 17 September 2026** | `claude/boss-ledger` — the cord and the returns (the simulation), then `claude/ledger-look` — the cord itself (the look), landed separately. Chosen as the first boss whose damage travels **the other way**: a shot the pair lands comes back down a violet cord into their own hull four beats later, at a column that moves along the hull with every return, and the guard window they have used on the field all game is pointed at something they caused. The one thing the design cannot have as written is the accumulating scar — *every hull damage fails the wave* (the owner, 12 September 2026, `sim/wave-fail.ts`) — so a missed ward is the lost wave it is everywhere else in this game, and step 14's scripted unwarded hit is reworked: the fifth return is the one the pair is told to **let through** rather than to take, and it tears the cord out of the ship instead of scarring it |
| [§9 THE SURGE](#9-the-surge--whether-you-can-stop) | **taken, 17 September 2026** | `claude/boss-implementation-e3cfff` — the bulb and the lift (the simulation), then the seam (the look), landed separately. Chosen as THE SINEW's split at the other verb, which the design says must follow it or not be built: the same sum the pair cannot read whole, passed by two lifts within a beat rather than a hold |
| the other three | free | — |
