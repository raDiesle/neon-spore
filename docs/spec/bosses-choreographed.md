...
| [§6 THE CURTAIN](#6-the-curtain--what-it-is-standing-in-front-of) | **taken, 17 September 2026** | `claude/boss-implementation-e3cfff` — the membrane and the core (the simulation), then the fabric (the look), landed separately. Chosen as the nearest of the eight to the owner's third kind: both hands on the boss's own body, shoving it aside |
| [§4 THE TASTER](#4-the-taster--what-you-have-already-spent) | **taken, 17 September 2026** | `claude/boss-taster` — the ledger and the fan (the simulation), then the blades (the look), landed separately. Chosen for the reusable the page itself says THE MOTHER has been waiting for: a rolling per-colour count of the pair's own shots, hashed, which nothing shipped has. The inverted colour rule gets its own guide screen, because it is the first mechanic in the game that would be wrong to leave discoverable |
| the other six | free | — |
