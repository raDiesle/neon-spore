...
| [§2 THE ORRERY](#2-the-orrery--whether-you-can-agree-on-when) | **taken, 17 September 2026** | `claude/boss-orrery` — three lanes, named before the first was started: **the rings** (the three orbits, the shaft, the shot that breaks a ring, the core's own fire, the lance that finishes it), then **the hand on the ring** (a bearing drag on the outermost unbroken ring, `crank`'s thousandths-of-a-turn on the field rather than the panel), then **the look** (three beaded orbits, the per-seat truth, the shaft of light). The first is the lane now running |
