...
| [§10 THE BATON](#10-the-baton--whose-turn-is-it) | **built, 16 September 2026** — **taken again, 17 September 2026** | `claude/boss-implementation-e3cfff` — the simulation, then the look. Worked and written up as [bosses](bosses.md) §11.18 (wave 68). **Step 7 and the beam landed** (`8be1c9fe`): a shot at anything is her turn spent from the tick it leaves, and the lance beam meets the bead the way a bolt does; no look, the grey panel already draws it. **The second bead landed** (`b97a312a`, step 9): it lights after three dark sockets, the trigger sends the longest sitter, a bolt takes the lowest bead, the lead waits in the last socket and the two merge. **Taken next, the same lane: the second bead's look** — the twin told apart from the first, the merged bead brighter; the merged final flight (steps 12–13) after it |
