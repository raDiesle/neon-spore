...
| [§16 THE INSTAR](#16-the-instar--whether-two-different-hands-can-finish-one-beat) | **taken, 17 September 2026** | `claude/tutorial-boss-onscreen-actions-07cc80` — the engine (`BossSequenceStep`, the beat list read by index) and the simulation, then the look. The owner's ask of 17 September 2026: a boss with no control set, only marks on its own body, answered one seat at a time or both at once |
