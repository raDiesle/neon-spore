...
| THE FLIP (`packages/sim/src/flip.ts`, a fault rather than a boss) | **built, 17 September 2026** — **taken for the rehearsal, 18 September 2026** | the fault and the mirrored field are shipped (wave `theFlip`, act 10, `render/src/field-flip.ts`). `claude/boss-implementation-e3cfff` is writing its rehearsal — a film of a screen that turns, shown by turning and then by being believed |
