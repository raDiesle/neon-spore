...
| Concept | State | Lane |
|---|---|---|
| [§17 THE FILAMENT](#17-the-filament--whether-you-can-follow-a-line-that-is-still-being-drawn) | **taken, 18 September 2026** | `claude/boss-implementation-e3cfff` — the simulation first (`TraceDrag` as a `drag` at a `filament` target whose progress is the tiles it has passed, hashed; the seven filaments, the draw a tile a beat, the follow within `filamentGapTiles`, the snap, the collision and the slip), then the look. `claude/boss-hints-mechanics-5b5a9f` wrote it out of the second brief and did not take it |
