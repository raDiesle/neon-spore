...
| [§11 THE LEAD](#11-the-lead--where-it-will-be) | **taken, 17 September 2026** | `claude/boss-implementation-e3cfff` — the pace and the flight (the simulation), then the lean (the look), landed separately. Chosen because every part is shipped and it is the one concept on the page whose split is arithmetic across two seats — her column, his direction, the lead computed out loud |
| the other two | free | — |
