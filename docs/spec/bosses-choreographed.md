...
| THE HUSK (`packages/sim/src/pod-types.ts` `husk`, a pod rather than a boss) | **built** — **taken for the rehearsal, 18 September 2026** | the hollow pod, its deflation and its mark are shipped (wave `theHusk`, act 10, `render/src/husk-mark.ts`, `husk-deflate.ts`). `claude/boss-implementation-e3cfff` is writing its rehearsal — the other seat's page, the maw left shut, the cost last |
