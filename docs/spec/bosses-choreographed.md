...
| [§15 THE SCUTTLE](#15-the-scuttle--a-boss-racing-you-to-its-own-death) | **taken, 17 September 2026** | `claude/boss-implementation-e3cfff` — the parts and the cadence (the simulation: the part list with a live index, a part loosening over three beats and thrown as an ordinary arrival, the live part struck off while it hangs, the pod that slows the cadence, the tightening as it thins, the wind-up over `lancePrimeBeats` that only the standing beam ends), then the look (the thinning body and its sockets, the live part in the ammunition colour, the wind-up and the collapse), landed separately |
| §12 THE ANTIPHON | free | — |
