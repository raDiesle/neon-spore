...
| Concept | State | Lane |
|---|---|---|
| [§18 THE GIMBAL](#18-the-gimbal--whether-the-same-turn-means-the-same-thing-to-both-of-you) | **taken, 22 September 2026** | `claude/queue-the-gimbal-is-written-and-nobody-has-built-its-s` — lane one, the simulation: `gimbalOuter` and `gimbalInner` as two `BearingDrag` members of `drag-targets.ts`, read the way `crank.ts` and `orrery-hand.ts` read theirs; the mirrored bearing, one true ring drawn the way each seat's own face would show it; the three alignments and the teeth they shear; the leaking seam of row 9. Lane two, the look, is its own `LOCAL ONLY` item on `docs/queue.md` and is not started here |
