...
| [§34 THE CYST](#34-the-cyst--a-pulse-stilled-by-one-hand-so-the-other-can-close-it) | **lane one landed, 26 September 2026** — the touch sender and lane two open | The simulation is in as wave 113 THE CYST and `docs/spec/bosses.md` §11.51: `cystFreezeLeft` and `cystFreezeRight` heard as THE VALVE hears its pin, an edge, and `cystFlankLeft` and `cystFlankRight` heard as THE VISE hears a lobe, `fromMilli` the gap; each side's two on different seats, the freezing hand always the pinching hand's partner. A script authored on the wave of a flank a seat, then fire and a guard in turn; a lit flank waits for the partner's tap, and only a stilled flank's pinch is counted; both flanks cracked bare the core; a guard is a flank step on a cracked flank, and one run out reseals the core; a fire step the ordinary shot into the middle column in its colour. Two cracks and three hits as the health. **Eight departures are argued by name** in §11.51: the shudder is a rule rather than a field, a tap and its pinch are one step, a pinch is given grace, a pinch already shut counts from the tap, a guard is one flank at a time, a guard run out is asked again, a fire step run out is the wave, and the targets are named for the body with THE SLOW off a shot. The fourteen sounds are bound; nothing sends a tap or a pinch from a touch yet, nothing of it is drawn, and there is no autopilot hand |
