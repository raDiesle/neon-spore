...
## 11.8 THE CLAW — the one where nothing has a name

> The one where the only one who can see the wrecks is the only one who cannot
> touch anything.

A ninth boss for a ninth question, and it is THE FLEET's own split turned
round. That fight gives the map to the seat holding the trigger, so the whole
of it is one person getting a coordinate out of their mouth. This one gives the
map to the seat with **no controls at all** — and takes the coordinates away.

**A rail and a row of sockets.** `cfg.clawCells` of them, nine by default, with
a claw hanging from a rail above. Player 1 slides it one socket a press with
LEFT and RIGHT and drops it with GRAB; player 2 has no button on the panel.
Buried in the sockets are `cfg.clawPods` pods and `cfg.clawRocks` rocks, dealt
from the seeded rng, and only player 2's screen shows what is in which. Raise
every pod and the round is over.

**It is deliberately not `cfg.cols` wide.** The field's eleven columns come
with a vocabulary — a column is a thing this game has spent an evening teaching
a pair to count and to name — and a rail as wide as the field would invite them
to reach for the wave's own words on a screen where nothing is labelled. Nine
sockets is a row nobody can address.

**And nothing is written on it.** THE FLEET's chart is lettered A to K across
and numbered 1 to 10 down, for the reason 11.6 gives at length: a name survives
being said once across a voice delay. Here there is no name, so a direction
cannot be said once. "Two left" is the only sentence available, and it is only
true until the next thing happens.

**The wrecks shift, and that is the round.** Every `cfg.clawDriftBeats` — six,
which is 3.75 seconds at 96 BPM, against a spoken exchange of 2.1–3.6 s
([latency](latency.md)) — one wreck with a free socket beside it moves into it,
and only player 2 sees it go. So the field moves about once per sentence: often
enough that a direction goes stale while it is being said, rarely enough that
saying one is still worth doing. A wreck never shifts *onto* another; it moves
into a hole, which is why the two sockets the deal leaves empty are load-bearing
rather than spare.

**A wrong grab costs the hull.** A rock raised takes `damageClawRock` in the
middle column, on the spot, and running out of `clawRoundBeats` takes
`damageClaw` — the same call THE GAUGE, SNAKE, THE MAZE, THE FLEET and PINBALL
all make when a round with no field of its own has to cost the ship something.
The scar is on the hull when the field comes back, which is what makes it read
rather than merely count. Every grab spends its socket, an empty one included:
a field that put the wreck back would let the pair sweep the rail end to end
and find everything without saying a word.

**Both screens are told what came up.** The seat that cannot see gets POD, ROCK
or EMPTY over the socket for the length of the rest, and that is deliberate
rather than a leak in the split: a claw that came up empty and said nothing is
a round with no feedback in it for the one person who has to act, and a pilot
who never learns what he pulled cannot tell a sentence he misheard from a wreck
that moved. It says what came up; it never says what is still down there.

**The empty half of the panel is the argument.** Every other control set in the
game puts both seats on it, and `test/control-sets.test.ts` holds that rule for
every panel but this one, by name. Player 2's half of THE CLAW is *speech*,
which is this game's actual control scheme rather than a substitute for one —
and a button for her would have to be a piece of the machine, which is the
round. It is the one deliberate exception, and the test says so in as many
words so a second one has to come back and argue for itself.

`ClawState` is in the `BossState` union. The wreck field's arithmetic — the
deal, the drift and the questions asked of a socket — is
`packages/sim/src/claw-field.ts`, the state and the two verbs `claw.ts`, the
clock and the four phases `claw-round.ts`; its numbers are `config-claw.ts` and
its panel is the `claw` control set. The picture is two files —
`packages/render/src/claw-rig.ts` for the rail, the sockets and the claw,
`claw-round.ts` for the stage — and it is **slabs and glyphs, never blobs**
([interludes](interludes.md)): every line of it is drawn corner to corner, with
no smoothing and no wobble, because the pilot is working a machine and a grown
outline would be a creature holding something. Its wave is `THE CLAW` and its
sentence is the epigraph.

What has **not** been looked at by a human is the tempo: whether six beats
between shifts is a field that keeps a pair talking or one that makes them
give up, whether nine sockets is a row two people can agree about at all
without counting from an end every time, and whether a seat with no button
reads as concentrating or as excluded. All three are questions about a room
with two people in it, and no test can answer any of them.
