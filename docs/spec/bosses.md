...
> **Status: this page is ordered by what is left to do.** What is still in
> somebody's hands stands first — a boss whose simulation landed and whose look
> is not written — then what is finished, and a design that shipped and was
> taken out again is last. **The `11.n` numbers are not in sequence
...
Written at boss scale on 27 August 2026 and kept here when the page they were
written on was deleted. [transfers](transfers.md#the-filter)'s five apply to
anything; these three apply only to a body a third of the screen wide, and
[bosses-choreographed](bosses-choreographed.md#the-filter-these-fifteen-had-to-pass)
adds two more for a scene.
...
1. **A fourth boss asks a fourth question.** The Queen is about *what you
   ...
   hands are free to do*. A second boss built on a shipped coupling is a
   re-skin, however different its body.
2. **A boss holds a row, or it is a fixture.** Nothing on the field travels,
   the bosses included: the Queen paces one row, THE MIRROR hangs at the top,
   The Warden never moves at all.
...
on the exclusion list, THE SCUTTLE's case): `act-7g.ts`'s "THE HIVE" has no
...
(`showsHiveColor`, `showsHiveSwell`, `render/view-role-clocks-b.ts`).
...
**The look.** A mass of wax hangs over the top of the field above row 0,
...
It fills its wave (`bossFillsWave`): `act-7e.ts`'s "THE CURTAIN" has no
...
**It is a fixture, not a body.** `bossFillsWave === false`, so the wave's own
arrivals fall under it (`content/src/waves/act-7f.ts`, "THE TASTER") — rocks
...
fill its wave (`bossFillsWave`): `act-7f.ts`'s "THE SINEW" carries its own
...
`content/src/waves/act-7g.ts`, carries its own eight arrivals — three rocks and
...
(`bossFillsWave`): `act-7g.ts`'s "THE SURGE" carries its own arrivals,
...
(`content/waves/act-7g.ts`), and the bulb's is the same rock arriving on the
...
(`bossFillsWave`): `act-7g.ts`'s "THE LEAD" carries its own arrivals, thin
...
the exclusion list): `act-7g.ts`'s "THE SCUTTLE" has no entries at all,
...
one of the nineteen after it is struck the beat after it comes loose, in
the column the strip found (`atBoss`, `scuttleNextCol` in
`sim/boss-answer.ts`) and the colour the navigator sees, the third one with
the wrong colour first for the rebuff. The twins are thrown regardless of
the strike, and each is shot at the top of its column in the gap between
two strikes; both pods are struck where they hang, at eleven and two left,
because a thrown pod's landing column is the rng's and rarely one an author
can put the cannon under. Cyan is held from the wind-up's own beat and the
beam is standing a beat before the throw; the frame is out three beats on.
Seed 17 is the one thing chosen: a seed that throws a pod, or lets a twin
