...
| **Limpet** | HOOK COLONY's round base in the malfunction's arc-blue, a rim of hooklets all curled the same way; squatting on the shield's plate with the hooks turned down into the plating | **a fault and never a wave's body** (15 September 2026): the lantern at the top of the field fires it at the plate very fast, like a harpoon, and it clamps on wherever the plate is — no fall, no lane, nothing to evade, and a shot is spent on it. From that beat the plate has to keep moving and is judged *between* beats: a plate that has not been found in a new column for `harpoonStillBeats` loses the round, a heavy hit on the hull at its column. The pencil's own length is how long it stays; when that runs out the line is reeled home. A square, a code, a timer and **MOVE SHIELD!** are drawn on the body, and the same word is under player 1's dial — the seat that cannot move it |
| **Leech** | CALTROP in the same arc-blue, four needles off a round body; on the cannon's swelling with the needles driven in | THE LIMPET's twin on the cannon, and a fault on the same terms: fired at the cannon, held there for the pencil's length, and lost the moment the cannon has stood still for `harpoonStillBeats`. **MOVE CANNON!** on the body and under player 2's dial, which is the seat that cannot move it. What both of them used to be — a fall down a lane, a fuse in lights, and a shake that got them off — is on `ideas.md` |
