...
export const ARMOURED_DRAFTS: CatalogueEntry[] = [
  ...
  },
  {
    ...
    slot: "creature",
    owner:
      "THE HASP, taken 23 September 2026, as each of its three clasps, the two halves pinned over a wheel-hub where the interior was left out (`render/hasp-shape.ts`); and THE MANTLE, 26 September 2026, as its two valves with THE SLATER's lapped plates laid in them (`render/mantle-shape.ts`). Before that: converted from a beetle's elytra, which is the plainest version in nature of a hard thing that opens — shut and unremarkable most of the time, hinged up before anything happens. It is here because THE LURE's fold was the only silhouette in this game that changed for a reason, and one is not a vocabulary. What is left behind is the whole interior: a real one opens onto folded wings and a soft abdomen, and drawing those would be two mistakes — the cards fill with `evenodd`, so an inner loop punches a hole through the halves as they close over it, and `guarded` already draws a body beside a separate piece. What is only here is the split, so that is all it draws. It carries TWITCH because a body that waits and then flicks is what a telegraph is, and the two clocks are deliberately not in step: if the card only reads while they happen to agree, it is not reading",
  },
  {
