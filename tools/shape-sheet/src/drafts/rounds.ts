...
export const ROUND_DRAFTS: CatalogueEntry[] = [
  {
    ...
    slot: "field",
    owner:
      "THE CLAW, which is built — and built as a **control set** rather than as the round this card was drawn at, which is the more interesting half of the story. The idea was a claw on a rail over its own screen; what shipped is the ship's own cannon replaced by this shape, reaching up an ordinary column of an ordinary field (`packages/render/src/reach-arm.ts`). The card survived the change because the thing it says is a *grip on a shaft*, and neither the rail nor the wreck field was ever what made it worth drawing. The gape is in the contour and not in the motion, which is why the picture survived at all: an own-motion can move a body about and cannot close a hand. What it must not be read as is THE MOTHER, which is also arms around an opening: hers is a hole with the field showing through the middle of a body, and this has no inside at all below the shaft",
  },
  {
