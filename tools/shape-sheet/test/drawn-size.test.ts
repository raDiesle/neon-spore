 ...
 *
 * The catalogue has grown eight bodies since — THE SHELL, and then the seven
 * converted off other games' screenshots in `drafts/tower-defence.ts` — so the
 * counts read 56, 39 and 20 rather than 49, 32 and 17. The finding is the
 ...
 * of small features is exactly the kind of body that loses most when the frame
 * narrows.
 *
...
describe("drawn size against the 20-26 px floor", () => {
  it("has the 56 square cards the catalogue now holds", () => {
    expect(SQUARE.length).toBe(56);
  });
