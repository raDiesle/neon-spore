...
const SLOT_POSE: Record<string, string> = {
  ...
  // A layer over a body giving way, and the first poses here with a hand kept
  // on the world after it is built (`Pose.hand`, `poses-layers.ts`): a rind
  // sheds on a shot and a lid opens under a pull, and neither happens on a
  // page where nobody fires and nobody pulls. The rind's own body is judged
  // on the same shed, as the layers come off it.
