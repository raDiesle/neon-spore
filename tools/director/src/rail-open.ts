 ...
 * scanning it a series of departures. So the row grows two small targets
 ...
 * identical small blocks at phone size. WAVE and MAP are the header's own names
 * for the two views (`index.html`), so the button says exactly where it goes.
 *
 * On a desktop all four columns are on screen at once, so the two buttons are
 * hidden by the stylesheet and this file's `scrollIntoView` is what a press
 * would have been worth there anyway.
 */

/** Where a row can send you, and the word that says so. */
const WAYS = [
...
/**
 * The two buttons for the row at `i`. `select` is the list's own selection —
