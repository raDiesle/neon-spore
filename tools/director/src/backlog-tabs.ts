 ...
 *
 * Three of them now — SHAPES, GUIDES, OTHER GRAPHICS — and each is expensive
 * in its own way: thirty-odd animated contours to fit, thirty-two posed
 * worlds to draw, two live renderers against one stepped world plus a baked
 * atlas to fetch and decode (OTHER GRAPHICS carries both the raster page and
 * the former ALTERNATIVES page — `raster-page.ts`, `versus-page.ts`). None of
 * it is work a session that opened the sheet to read the bestiary should pay
 * for.
 *
...
export function mountLazyTabs(): void {
  ...
  // else. The other two carry their own `drawn` flag.
