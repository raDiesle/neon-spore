 ...
 * - **`tools/hooks/bun-pin.ts`'s `WANTED`** is what the web image's bun is
 *   pinned *to* by the session-start hook and what `bun run land` refuses a
 *   bun below, and it is the reader that had already drifted: it held `1.4.2`
 *   as a literal while this file said `1.4.0`. It reads `.bun-version` now,
 *   and the row below is what says the derivation still reaches this file — a
 *   module moved to another directory would quietly fall back to its no-op,
 *   pin nothing and refuse nothing.
 *
...
describe("the Bun version this project expects", () => {
  ...
  it("is the version the session-start hook pins to and the landing refuses below", async () => {
    const pinned = (await text(".bun-version")).trim();
    expect(WANTED, "tools/hooks/bun-pin.ts does not read .bun-version").toBe(pinned);
  });
